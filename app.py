...
class BinanceFutures:
    def __init__(self):
        ...
        log.info(
            f"🔧 CONECTADO A BINANCE FUTURES {'TESTNET' if testnet else 'MAINNET'} - 20x LEVERAGE"
        )
    ...
    def is_symbol_tradable(self, symbol: str) -> bool:
        ...
        try:
            ...
            if "contracts" not in s_info or not s_info["contracts"]:
                return False
...
class TradingBot:
    ...
    def get_klines_for_symbol(
        ...
        if not self.api.is_symbol_tradable(symbol):
            log.warning(f"Intentando obtener klines para símbolo no tradable: {symbol}")
            return None
    ...
    def check_signal(self, df: pd.DataFrame, symbol: str) -> Optional[str]:
        ...
        if len(df) >= 28:
            ...
            if adx < config.MIN_ADX_FOR_TREND:
                log.info(
                    f"↔️ Mercado lateral para {symbol} (ADX: {adx:.1f} < {config.MIN_ADX_FOR_TREND}), omitiendo"
                )
                return None
...
def api_update_config():
    ...
    try:
        ...
        return jsonify(
            {"status": "config_updated", "message": "Configuración actualizada"}
        )
...
def api_close_position():
    ...
    try:
        ...
        socketio.emit(
            "log_update",
            {"message": f"Posición cerrada manualmente: {symbol}", "level": "INFO"},
        )
...
def api_manual_trade():
    ...
    try:
        ...
        socketio.emit(
            "log_update",
            {"message": f"Trade manual ejecutado: {side} {symbol}", "level": "INFO"},
        )

        return jsonify(
            {"status": "success", "message": f"Trade manual {side} {symbol} ejecutado"}
        )
