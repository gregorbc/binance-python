...
try:
    ...
    DRL_ENABLED = False
    print(
        "WARNING: TensorFlow not found. Deep Learning features will be disabled. By gregorbc@gmail.com"
    )

from sqlalchemy import case, desc, func

try:
    from database import AccountBalance, PerformanceMetrics, SessionLocal, Trade

    DB_ENABLED = True
except ImportError:
    DB_ENABLED = False
    print(
        "WARNING: 'database.py' not found. Database features will be disabled. By gregorbc@gmail.com"
    )
...
class CONFIG:
    ...
    # Changed from 2.0 to 3.5 By gregorbc@gmail.com
    RISK_PER_TRADE_PERCENT: float = 3.5
    ...
    MIN_VOLUME_CONFIRMATION: float = (
        1.2  # Changed from 1.8 to 1.2 By gregorbc@gmail.com
    )
    ...
    # Changed from 1.0 to 0.5 By gregorbc@gmail.com
    REINVESTMENT_THRESHOLD: float = 0.5
    # Changed from 0.5 to 1.0 By gregorbc@gmail.com
    CAPITAL_GROWTH_TARGET: float = 1.0
    # Capital inicial (se actualizará con el real)
...
class CapitalManager:
    ...
    def update_balance(self, force: bool = False) -> bool:
        ...
        if force or (
            current_time - self.last_balance_update > 300
            ...
            if new_balance > 0:
                if self.current_balance == 0:
                    ...
                    log.info(
                        f"💰 Capital inicial: {new_balance:.2f} USDT By gregorbc@gmail.com"
                    )
    ...
    def reinvest_profit(self, profit: float):
        ...
        if profit > 0 and self.should_reinvest(profit):
            ...
            log.info(
                f"🔄 Reinvirtiendo {profit:.2f} USDT en el capital By gregorbc@gmail.com"
            )
...
class TelegramNotifier:
    ...
    def notify_trade_opened(
        self, symbol: str, side: str, quantity: float, price: float, balance: float
    ):
    ...
    def notify_balance_update(
        self, balance: float, profit: float, profit_percentage: float
    ):
...
class BinanceFutures:
    def __init__(self):
        ...
        if not api_key or not api_secret:
            raise ValueError(
                "API keys not configured. Set BINANCE_API_KEY and BINANCE_API_SECRET environment variables By gregorbc@gmail.com"
            )

        self.client = Client(api_key, api_secret, testnet=testnet)
        log.info(
            f"🔧 CONNECTED TO BINANCE FUTURES {'TESTNET' if testnet else 'MAINNET'} By gregorbc@gmail.com"
        )

        try:
            self.exchange_info = self.client.futures_exchange_info()
            log.info(
                "✅ Exchange information loaded successfully By gregorbc@gmail.com"
            )
        except Exception as e:
    ...
    def ensure_symbol_settings(self, symbol: str):
        ...
        try:
            ...
            if e.code == -4046 or "No need to change margin type" in e.message:
                ...
                log.warning(
                    f"Margin type set warning for {symbol}: {e} By gregorbc@gmail.com"
                )
        except Exception as e:
            log.error(
                f"An unexpected error occurred setting margin type for {symbol}: {e} By gregorbc@gmail.com"
            )
    ...
    def _safe_api_call(self, func, *args, **kwargs):
        for attempt in range(config.MAX_API_RETRIES):
            try:
                ...
                if e.code == -4131:
                    log.warning(
                        "PERCENT_PRICE error (-4131) in order. Volatile or illiquid market. Skipping. By gregorbc@gmail.com"
                    )
                    return None
                elif e.code in [-2011, -2021]:
                    log.warning(
                        f"API order error ({e.code}): {e.message} By gregorbc@gmail.com"
                    )
                else:
                    log.warning(
                        f"API non-critical error: {e.code} - {e.message} By gregorbc@gmail.com"
                    )
                    if attempt == config.MAX_API_RETRIES - 1:
                        log.error(
                            f"Final API error after all retries: {e.code} - {e.message} By gregorbc@gmail.com"
                        )
            except Exception as e:
                with state_lock:
                    app_state["connection_metrics"]["api_retries"] += 1
                ...
                if attempt == config.MAX_API_RETRIES - 1:
                    log.error(
                        f"Final general error after all retries: {e} By gregorbc@gmail.com"
                    )
    ...
    def smart_retry_api_call(self, func, *args, **kwargs):
        ...
        for attempt, delay in enumerate(retry_delays):
            try:
                ...
                if attempt > 0:
                    log.info(
                        f"✅ Reintento exitoso después de {attempt} intentos By gregorbc@gmail.com"
                    )
                ...
                if e.code in [-1013, -2010, -2011]:
                    log.error(
                        f"Error no recuperable: {e.message} (código: {e.code}) By gregorbc@gmail.com"
                    )
                    break

                log.warning(
                    f"Reintentando en {delay}s (intento {attempt + 1}/{len(retry_delays)}) By gregorbc@gmail.com"
                )
                time.sleep(delay)
                ...
                last_exception = e
                log.warning(
                    f"Error general, reintentando en {delay}s (intento {attempt + 1}/{len(retry_delays)}) By gregorbc@gmail.com"
                )
                time.sleep(delay)

        log.error(
            f"❌ Todos los reintentos fallaron: {last_exception} By gregorbc@gmail.com"
        )
        return None
    ...
    def close_position(self, symbol: str, position_amt: float) -> Optional[Dict]:
        ...
        if config.DRY_RUN:
            log.info(
                f"[DRY_RUN] close_position {symbol} {position_amt} By gregorbc@gmail.com"
            )
            return {"mock": True, "orderId": int(time.time() * 1000)}
    ...
    def cancel_order(self, symbol: str, orderId: int) -> Optional[Dict]:
        if config.DRY_RUN:
            log.info(
                f"[DRY_RUN] cancel_order: {orderId} for {symbol} By gregorbc@gmail.com"
            )
            return {"mock": True}
        ...
        try:
            ...
            log.warning(
                f"Could not cancel order {orderId} for {symbol}: {e} By gregorbc@gmail.com"
            )
            return None
...
class TradingBot:
    ...
    def get_top_symbols(self) -> List[str]:
        ...
        suitable_symbols.sort(
            key=lambda x: (float(x["quoteVolume"]), float(x["lastPrice"])), reverse=True
        )
    ...
    def calculate_adx(self, df: pd.DataFrame, period: int = 14) -> float:
        ...
        try:
            ...
            log.error(
                f"Error calculando ADX para {symbol} con talib: {e} By gregorbc@gmail.com"
            )
    ...
    def check_signal(self, df: pd.DataFrame, symbol: str) -> Optional[str]:
        ...
        if adx < 15:  # Changed from 20 to 15
            log.info(
                f"↔️ Mercado lateral detectado para {symbol} (ADX: {adx:.1f}), evitando señales By gregorbc@gmail.com"
            )
            return None
        ...
        if config.VOLUME_WEIGHTED_SIGNALS:
            ...
            if volume_ratio < config.MIN_VOLUME_CONFIRMATION:
                log.info(
                    f"📉 Señal {signal} descartada por volumen insuficiente: {volume_ratio:.2f} By gregorbc@gmail.com"
                )
                return None
            ...
            try:
                ...
                if df_higher is not None and len(df_higher) > 20:
                    ...
                    if higher_tf_signal != signal:
                        log.info(
                            f"⏭️ Señal {signal} en {symbol} descartada por falta de alineación con TF superior ({higher_tf_signal}) By gregorbc@gmail.com"
                        )
                        return None
            except Exception as e:
                log.error(
                    f"Error verificando TF superior para {symbol}: {e} By gregorbc@gmail.com"
                )
        ...
        if config.BTC_CORRELATION_FILTER and symbol != "BTCUSDT":
            ...
            if btc_df is not None and len(btc_df) > 20:
                ...
                if btc_signal and btc_signal != signal:
                    ...
                    strength *= 0.6
                    log.info(
                        f"⚠️ Señal {signal} en {symbol} con señal contraria en BTC, fuerza reducida a {strength:.2f} By gregorbc@gmail.com"
                    )
        ...
        if strength < config.MIN_SIGNAL_STRENGTH:
            log.info(
                f"📉 Señal {signal} descartada por fuerza insuficiente: {strength:.2f} By gregorbc@gmail.com"
            )
            return None

        log.info(
            f"📶 Señal {signal} con fuerza: {strength:.2f}, confirmaciones: {confirmations}/3 By gregorbc@gmail.com"
        )
        return signal
    ...
    def update_ai_model(
        ...
        try:
            ...
            log.error(
                f"Error updating AI model for {symbol}: {e} By gregorbc@gmail.com"
            )
        ...
        with state_lock:
            ...
            if config.AI_TRAILING_ADJUSTMENT and DB_ENABLED:
                ...
                if performance_data:
                    ...
                    if trailing_adjustment != 1.0:
                        log.info(
                            f"🤖 AI Trailing Adjustment for {symbol}: {trailing_adjustment:.2f} (Volatility: {performance_data.get('market_volatility', 0):.2f}%) By gregorbc@gmail.com"
                        )
            ...
            if position_side == "LONG":
                ...
                if (
                    ...
                    log.info(
                        f"🔔 Trailing stop activado para {symbol} (adj: {trailing_data['trailing_adjustment']:.2f}) By gregorbc@gmail.com"
                    )
                if trailing_data["activated"]:
                    ...
                    if current_price <= trailing_data["current_stop"]:
                        log.info(
                            f"🔴 Cierre por trailing stop: {symbol} @ {current_price} (Stop: {trailing_data['current_stop']}) By gregorbc@gmail.com"
                        )
                        should_close = True
            else:
                if current_price < trailing_data["best_price"]:
    ...
    def check_fixed_sl_tp(self, symbol: str, position: Dict, current_price: float):
        ...
        with state_lock:
            ...
            if (config.AI_SL_ADJUSTMENT or config.AI_TP_ADJUSTMENT) and DB_ENABLED:
                ...
                if performance_data:
                    ...
                    if sl_adjustment != 1.0 or tp_adjustment != 1.0:
                        log.info(
                            f"🤖 AI SL/TP Adjustment for {symbol}: SL={sl_adjustment:.2f}, TP={tp_adjustment:.2f} By gregorbc@gmail.com"
                        )
            ...
            if "sl_price" not in sl_tp_data:
                ...
                log.info(
                    f"Initialized SL/TP for {symbol}: SL @ {sl_price:.4f} (adj: {sl_adjustment:.2f}), TP @ {tp_price:.4f} (adj: {tp_adjustment:.2f}) By gregorbc@gmail.com"
                )
            ...
            if position_side == "LONG":
                if current_price <= sl_price:
                    log.info(
                        f"🔴 Cierre por STOP LOSS: {symbol} @ {current_price} (SL: {sl_price}) By gregorbc@gmail.com"
                    )
                    return "SL"
                elif current_price >= tp_price:
                    log.info(
                        f"🟢 Cierre por TAKE PROFIT: {symbol} @ {current_price} (TP: {tp_price}) By gregorbc@gmail.com"
                    )
                    return "TP"
            else:
                if current_price >= sl_price:
    ...
    def check_time_based_exit(self, symbol: str, position: Dict) -> bool:
        ...
        if hours_held > config.MAX_POSITION_HOLD_HOURS:
            log.info(
                f"⏰ Cierre por tiempo: {symbol} held for {hours_held:.1f} hours By gregorbc@gmail.com"
            )
            return True
        return False
    ...
    def check_momentum_reversal(
        ...
        if position_side == "LONG":
            ...
            if price_trend and rsi_trend and current_rsi > 70:
                log.info(
                    f"↘️ Cierre por divergencia RSI bajista: {symbol} By gregorbc@gmail.com"
                )
                return True
        else:
            price_trend = df["close"].iloc[-1] < df["close"].iloc[-3]
            ...
            if price_trend and rsi_trend and current_rsi < 30:
                log.info(
                    f"↗️ Cierre por divergencia RSI alcista: {symbol} By gregorbc@gmail.com"
                )
                return True
    ...
    def check_balance_risk(self, account_info):
        ...
        if current_balance < config.MIN_BALANCE_THRESHOLD:
            log.warning(
                f"⚠️ Balance bajo: {current_balance:.2f} USDT (mínimo: {config.MIN_BALANCE_THRESHOLD} USDT) By gregorbc@gmail.com"
            )
            return True
        ...
        if exposure_ratio > 0.8:
            log.warning(
                f"⚠️ Exposición demasiado alta: {exposure_ratio:.2%} By gregorbc@gmail.com"
            )
        ...
        with state_lock:
            ...
            if app_state["balance_history"]:
                ...
                if drawdown > config.MAX_DRAWDOWN_PERCENT:
                    log.warning(
                        f"⛔ Drawdown máximo excedido: {drawdown:.2f}% By gregorbc@gmail.com"
                    )
                    return True
    ...
    def calculate_position_size(self, symbol: str, price: float) -> float:
        ...
        if df is not None and not df.empty and len(df) > 20:
            ...
            log.info(
                f"📊 Volatilidad {symbol}: {volatility:.4f}, Factor: {volatility_factor:.2f} By gregorbc@gmail.com"
            )
    ...
    def analyze_trading_performance(self, symbol: str):
        ...
        try:
            ...
            profit_factor = (
                total_win_pnl / total_loss_pnl if total_loss_pnl > 0 else total_win_pnl
            )
            ...
            if atr > strategy_optimizer.VOLATILITY_THRESHOLD:
                recommended_leverage = max(
                    recommended_leverage - strategy_optimizer.LEVERAGE_ADJUSTMENT_STEP,
                    strategy_optimizer.MIN_LEVERAGE,
                )
            ...
            log.info(
                f"📊 Análisis de rendimiento para {symbol}: WR: {win_rate:.2%}, PF: {profit_factor:.2f}, Lev. Rec: {recommended_leverage}x, Vol: {atr:.2f}%, Dur: {avg_trade_duration:.1f}m By gregorbc@gmail.com"
            )
            return asdict(metrics)

        except Exception as e:
            log.error(
                f"Error analizando rendimiento para {symbol}: {e} By gregorbc@gmail.com"
            )
            return None
        finally:
            if "db" in locals():
        ...
        try:
            ...
            for symbol, loss_count in recent_losing_trades:
                ...
                if total_trades > strategy_optimizer.MIN_TRADES_FOR_ANALYSIS:
                    ...
                    if loss_ratio > 0.7:
                        log.warning(
                            f"⚠️ Símbolo problemático detectado: {symbol} con {loss_ratio:.2%} de trades perdedores By gregorbc@gmail.com"
                        )

        except Exception as e:
            log.error(
                f"Error optimizando estrategia basada en pérdidas: {e} By gregorbc@gmail.com"
            )
        finally:
            if "db" in locals():
                db.close()
    ...
    def generate_daily_report(self):
        ...
        try:
            ...
            daily_trades = db.query(Trade).filter(Trade.timestamp >= start_of_day).all()
...
"""
            ...
            log.info(
                f"📊 Reporte diario generado: {total_trades} trades, P&L Diario: {total_pnl:+.2f} USDT By gregorbc@gmail.com"
            )
    ...
    def open_trade(self, symbol: str, side: str, last_candle):
        ...
        if not symbol_settings.get("enabled", True):
            log.info(
                f"⏭️ Símbolo {symbol} deshabilitado en configuración By gregorbc@gmail.com"
            )
            return
        ...
        if min_position_size * price > self.capital_manager.current_balance:
            log.info(
                f"⏭️ {symbol} requiere mínimo {min_position_size * price:.2f} USDT, capital insuficiente By gregorbc@gmail.com"
            )
            return
        ...
        if quantity <= 0:
            log.info(
                f"⏭️ Tamaño de posición inválido para {symbol} By gregorbc@gmail.com"
            )
            return
        ...
        if quantity < min_qty:
            log.info(
                f"⏭️ Cantidad {quantity} para {symbol} es menor que el mínimo {min_qty} By gregorbc@gmail.com"
            )
            return
        ...
        if notional_value < min_notional:
            log.info(
                f"⏭️ Valor nocional {notional_value:.2f} para {symbol} es menor que el mínimo {min_notional:.2f} By gregorbc@gmail.com"
            )
            return
        ...
        log.info(
            f"Attempting to place MARKET order for {side} {symbol} (Qty: {quantity}, Value: {notional_value:.2f} USDT) By gregorbc@gmail.com"
        )
        ...
        if order and order.get("orderId"):
            log.info(
                f"✅ MARKET ORDER CREATED: {side} {quantity} {symbol} By gregorbc@gmail.com"
            )
            ...
            log.error(
                f"❌ Could not create market order for {symbol}. Response: {order} By gregorbc@gmail.com"
            )

    def run(self):
        log.info(
            f"🚀 STARTING TRADING BOT v12.0 WITH REAL CAPITAL MANAGEMENT By gregorbc@gmail.com"
        )
        self.start_time = time.time()
        ...
        while True:
            ...
            try:
                self.cycle_count += 1
                log.info(
                    f"--- 🔄 New scanning cycle ({self.cycle_count}) - Capital: {self.capital_manager.current_balance:.2f} USDT --- By gregorbc@gmail.com"
                )
                ...
                if current_date != self.today:
                    ...
                    log.info(
                        "🔄 Nuevo día - Reiniciando métricas diarias By gregorbc@gmail.com"
                    )
                ...
                if self.cycle_count % 12 == 0:
                    ...
                    log.info(
                        f"🏛️ Market Regime: {self.market_regime} By gregorbc@gmail.com"
                    )
                ...
                if (
                    ...
                    log.info(
                        "🧹 Cleaning recent signals memory (cooldown). By gregorbc@gmail.com"
                    )
                ...
                if self.daily_pnl < -(
                    ...
                    log.warning(
                        f"⛔ Límite de pérdida diaria alcanzado: {self.daily_pnl:.2f} USDT By gregorbc@gmail.com"
                    )
                    time.sleep(config.POLL_SEC)
                    continue
                ...
                if low_balance:
                    log.warning(
                        "⏸️ Pausando nuevas operaciones por balance bajo By gregorbc@gmail.com"
                    )
                ...
                for symbol, position in open_positions.items():
                    try:
                        ...
                        log.error(
                            f"Error checking stops for {symbol}: {e} By gregorbc@gmail.com",
                            exc_info=True,
                        )
                ...
                if self.cycle_count % 6 == 0 and DB_ENABLED:
                    try:
                        ...
                        log.error(
                            f"Error en análisis de rendimiento: {e} By gregorbc@gmail.com"
                        )
                ...
                if num_open_pos < config.MAX_CONCURRENT_POS and not low_balance:
                    ...
                    log.info(
                        f"🔍 Scanning {len(symbols_to_scan)} symbols for new signals. By gregorbc@gmail.com"
                    )
                    ...
                    with ThreadPoolExecutor(
                        ...
                        for future in futures:
                            ...
                            if signal:
                                log.info(
                                    f"🔥 Signal found! {signal} on {symbol} By gregorbc@gmail.com"
                                )
                                ...
                                if len(open_positions) + 1 >= config.MAX_CONCURRENT_POS:
                                    log.info(
                                        "🚫 Concurrent positions limit reached. By gregorbc@gmail.com"
                                    )
                                    break
                ...
                log.error(
                    f"Error in main loop: {e} By gregorbc@gmail.com", exc_info=True
                )
    ...
    try:
        ...
        if DB_ENABLED:
            try:
                ...
                log.error(
                    f"DB Error recording trade for {symbol}: {e} By gregorbc@gmail.com"
                )
            finally:
        ...
        log.info(
            f"✅ Position closed: {symbol}, PnL: {realized_pnl:.2f} USDT, ROE: {roe:.2f}%, Reason: {close_type} By gregorbc@gmail.com"
        )
        socketio.emit("status_update", app_state)
        ...
        for key, value in data.items():
            if hasattr(config, key):
                try:
                    ...
                    log.warning(
                        f"Failed to set config field {key} with value {value}: {e} By gregorbc@gmail.com"
                    )
                    return (
                        jsonify(
                            {
...
def manual_trade():
    ...
    try:
        ...
        if order and (order.get("orderId") or order.get("mock")):
            log.info(
                f"MANUAL TRADE CREATED: {side} {quantity} {symbol} By gregorbc@gmail.com"
            )
            return jsonify(
                {
                    "status": "success",
...
def get_db_trade_history():
    ...
    try:
        ...
        total_losses = abs(
            db.query(func.sum(Trade.pnl)).filter(Trade.pnl < 0).scalar() or 0
        )
        ...
        result = {
            ...
            "performance_metrics": {
                ...
                "win_rate": (
                    (overall_stats.winning_trades / overall_stats.total_trades * 100)
                    if overall_stats.total_trades > 0
                    else 0
                ),
                "total_pnl": (
                    float(overall_stats.total_pnl) if overall_stats.total_pnl else 0
                ),
                "avg_win": float(overall_stats.avg_win) if overall_stats.avg_win else 0,
                "avg_loss": (
                    float(overall_stats.avg_loss) if overall_stats.avg_loss else 0
                ),
                "profit_factor": profit_factor,
...
def get_performance_by_symbol():
    ...
    try:
        ...
        result = [
            {
                ...
                "profit_factor": (
                    (float(stat.avg_win) / abs(float(stat.avg_loss)))
                    if stat.avg_loss and stat.avg_loss != 0 and stat.avg_win
                    else 0
                ),
        ...
        log.error(
            f"Error obteniendo rendimiento por símbolo: {e} By gregorbc@gmail.com"
        )
        return jsonify({"error": str(e)}), 500
    finally:
        if "db" in locals():
...
if __name__ == "__main__":
    ...
    log.info(
        "🚀 Starting Binance Futures Bot Web Application v12.0 with Real Capital Management By gregorbc@gmail.com"
    )
