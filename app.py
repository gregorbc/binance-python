...
⏰ {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}
    ...
    def notify_trade_closed(
        ...
        emoji = "🟢" if pnl >= 0 else "🔴"
        roe = (
            (pnl / (quantity * entry_price / leverage)) * 100 if entry_price > 0 else 0
        )
...
class TradingBot:
    ...
    def calculate_signal_strength(
        ...
        volume_ratio = (
            min(2.0, df["volume"].iloc[-1] / volume_avg) if volume_avg > 0 else 1
        )
        ...
        price_distance = (
            ema_distance / df["close"].iloc[-1] if df["close"].iloc[-1] > 0 else 0
        )
    ...
    def check_trailing_stop(self, symbol: str, position: Dict, current_price: float):
        with state_lock:
            ...
            activation_percent = (
                config.TRAILING_STOP_ACTIVATION * trailing_data["trailing_adjustment"]
            )
            stop_percentage = (
                config.TRAILING_STOP_PERCENTAGE * trailing_data["trailing_adjustment"]
            )
    ...
    def generate_daily_report(self):
        ...
        def report_operation():
            ...
            try:
                ...
                daily_trades = (
                    db.query(Trade).filter(Trade.timestamp >= start_of_day).all()
                )
                    ...
                    try:
                        ...
                        if exit_signal:
                            ...
                            if close_order:
                                ...
                                if (
                                    exit_signal.lower() in ["sl", "tp", "trailing"]
                        ...
                        if self.telegram_notifier.enabled:
                            self.telegram_notifier.notify_error(
                                f"Error verificando stops para {symbol}", str(e)
                            )
...
def close_position_api():
    ...
    try:
        ...
        if close_order:
            ...
            exit_price = (
                float(ticker["price"]) if ticker else float(position["entryPrice"])
            )
            pnl = _record_closed_trade(
                api, symbol, position, close_order.get("orderId"), "manual", exit_price
            )
            ...
            return (
                jsonify({"status": "error", "message": "Failed to send close order."}),
                500,
            )
...
def manual_trade():
    ...
    try:
        ...
        if not mark:
            return (
                jsonify({"status": "error", "message": "Unable to fetch mark price."}),
                500,
            )
