...
try:
    ...
    DRL_ENABLED = False
    print(
        "ADVERTENCIA: TensorFlow no encontrado. Las características de Deep Learning estarán deshabilitadas."
    )
...
class CONFIG:
    ...
    MIN_24H_VOLUME: float = 5_000_000  # Aumentado para mejor calidad
    ...
    HIGHER_TIMEFRAME_CONFIRMATION: bool = (
        False  # Desactivado temporalmente para debugging
    )
    ...
    MIN_ADX_FOR_TREND: float = 8.0
    RSI_OVERBOUGHT: float = 72.0
    RSI_OVERSOLD: float = 28.0
    MAX_VOLATILITY_PERCENT: float = 30.0
        ...
        if force or (current_time - self.last_balance_update > 240):
            ...
            if new_balance > 0:
                if self.current_balance == 0:
                    ...
                    log.info(
                        f"💰 Capital inicial: {new_balance:.2f} USDT (20x leverage)"
                    )
...
class TelegramNotifier:
    ...
    def send_message(
        self, message: str, parse_mode: str = "HTML", retry_count: int = 3
    ) -> bool:
        ...
        for attempt in range(retry_count):
            try:
                ...
                log.warning(
                    f"⚠️ Error enviando mensaje Telegram (intento {attempt + 1}/{retry_count}): {e}"
                )
...
class BinanceFutures:
    ...
    def is_symbol_tradable(self, symbol: str) -> bool:
        ...
        try:
            ...
            if contract_type not in ["PERPETUAL", "CURRENT_QUARTER", "NEXT_QUARTER"]:
                log.info(
                    f"Símbolo {symbol} no es un contrato válido. Tipo: {contract_type}"
                )
                return False
    ...
    def ensure_symbol_settings(self, symbol: str):
        if not self.is_symbol_tradable(symbol):
            log.warning(
                f"⏭️ Símbolo {symbol} no está disponible para trading, omitiendo configuración"
            )
            return
        ...
        try:
            ...
            log.info(
                f"✅ Apalancamiento configurado a {config.LEVERAGE}x para {symbol}"
            )
        except Exception as e:
        ...
        try:
            ...
            if e.code == -4046 or "No need to change margin type" in e.message:
                ...
                log.warning(
                    f"Advertencia al establecer tipo de margen para {symbol}: {e}"
                )
        except Exception as e:
            log.error(
                f"Error inesperado al establecer tipo de margen para {symbol}: {e}"
            )
            ...
            try:
                ...
                if e.code == -4131:
                    ...
                    if attempt == config.MAX_API_RETRIES - 1:
                        log.error(
                            f"Error final de API después de todos los reintentos: {e.code} - {e.message}"
                        )
            except Exception as e:
                with state_lock:
                    app_state["connection_metrics"]["api_retries"] += 1
                ...
                if attempt == config.MAX_API_RETRIES - 1:
                    log.error(
                        f"Error general final después de todos los reintentos: {e}"
                    )
    ...
    def smart_retry_api_call(self, func, *args, **kwargs):
        ...
        for attempt, delay in enumerate(retry_delays):
            try:
                ...
                log.warning(
                    f"Reintentando en {delay}s (intento {attempt + 1}/{len(retry_delays)})"
                )
                time.sleep(delay)
                ...
                last_exception = e
                log.warning(
                    f"Error general, reintentando en {delay}s (intento {attempt + 1}/{len(retry_delays)})"
                )
                time.sleep(delay)
    ...
    def get_symbol_filters(self, symbol: str) -> Optional[Dict[str, float]]:
        try:
            ...
            if s_info.get("status") != "TRADING":
                log.warning(
                    f"Símbolo {symbol} no está disponible para trading. Estado: {s_info.get('status')}"
                )
                return None
    ...
    def close_position(self, symbol: str, position_amt: float) -> Optional[Dict]:
        if not self.is_symbol_tradable(symbol):
            log.error(
                f"❌ No se puede cerrar posición para símbolo no tradable: {symbol}"
            )
            return None
...
class TradingBot:
    def __init__(self):
        ...
        if self.telegram_notifier.enabled:
            telegram_polling_thread = threading.Thread(
                target=self.telegram_handler.poll_commands, daemon=True
            )
    ...
    def get_top_symbols(self) -> List[str]:
        ...
        for t in tickers:
            ...
            # Excluir símbolos problemáticos
            if (
                symbol.endswith("USDT")
                and symbol not in config.EXCLUDE_SYMBOLS
                and symbol not in config.BLACKLIST_SYMBOLS
        ...
        # Tomar los mejores símbolos con buena liquidez
        top_symbols = []
        ...
        log.info(
            f"🔍 Encontrados {len(top_symbols)} símbolos válidos de {len(valid_tickers)} posibles"
        )
        return top_symbols
    ...
    def check_signal(self, df: pd.DataFrame, symbol: str) -> Optional[str]:
        ...
        # Verificar ADX para 20x - requisitos relajados
        if len(df) >= 28:
            adx = self.calculate_adx(df)
            # Para 20x leverage, permitir ADX más bajo pero con otras confirmaciones
            if adx < config.MIN_ADX_FOR_TREND:
                log.info(
                    f"↔️ ADX bajo para {symbol} (ADX: {adx:.1f}), pero continuando con otras verificaciones"
                )
        ...
        if "volatility" in df.columns and not pd.isna(df["volatility"].iloc[-1]):
            ...
            if volatility > config.MAX_VOLATILITY_PERCENT:
                log.info(
                    f"⚡ Alta volatilidad en {symbol}: {volatility:.1f}%, omitiendo"
                )
                return None
        ...
        if config.VOLUME_WEIGHTED_SIGNALS:
            ...
            if volume_ratio < config.MIN_VOLUME_CONFIRMATION:
                log.info(
                    f"📉 Señal {signal} descartada por volumen insuficiente: {volume_ratio:.2f}"
                )
                return None
            ...
            try:
                ...
                if df_higher is not None and len(df_higher) > 20:
                    ...
                    if higher_tf_signal != signal:
                        log.info(
                            f"⏭️ Señal {signal} en {symbol} descartada por falta de alineación con TF superior ({higher_tf_signal})"
                        )
                        return None
            except Exception as e:
        ...
        if config.BTC_CORRELATION_FILTER and symbol != "BTCUSDT":
            ...
            if btc_df is not None and len(btc_df) > 20:
                ...
                if btc_signal and btc_signal != signal:
                    ...
                    strength *= 0.6
                    log.info(
                        f"⚠️ Señal {signal} en {symbol} con señal contraria en BTC, fuerza reducida a {strength:.2f}"
                    )
        ...
        if strength < config.MIN_SIGNAL_STRENGTH:
            log.info(
                f"📉 Señal {signal} descartada por fuerza insuficiente: {strength:.2f}"
            )
            return None

        log.info(
            f"📶 Señal {signal} con fuerza: {strength:.2f}, confirmaciones: {confirmations}/3"
        )
        return signal
        ...
        with state_lock:
            ...
            if config.AI_TRAILING_ADJUSTMENT and DB_ENABLED:
                ...
                if performance_data:
                    ...
                    if trailing_adjustment != 1.0:
                        log.info(
                            f"🤖 Ajuste de Trailing por IA para {symbol}: {trailing_adjustment:.2f} (Volatilidad: {performance_data.get('market_volatility', 0):.2f}%)"
                        )
            ...
            if position_side == "LONG":
                ...
                if (
                    ...
                    log.info(
                        f"🔔 Trailing stop activado para {symbol} (adj: {trailing_data['trailing_adjustment']:.2f})"
                    )
                if trailing_data["activated"]:
                    ...
                    if current_price <= trailing_data["current_stop"]:
                        log.info(
                            f"🔴 Cierre por trailing stop: {symbol} @ {current_price} (Stop: {trailing_data['current_stop']})"
                        )
                        should_close = True
            else:
                if current_price < trailing_data["best_price"]:
    ...
    def check_fixed_sl_tp(self, symbol: str, position: Dict, current_price: float):
        ...
        with state_lock:
            ...
            if (config.AI_SL_ADJUSTMENT or config.AI_TP_ADJUSTMENT) and DB_ENABLED:
                ...
                if performance_data:
                    ...
                    if sl_adjustment != 1.0 or tp_adjustment != 1.0:
                        log.info(
                            f"🤖 Ajuste SL/TP por IA para {symbol}: SL={sl_adjustment:.2f}, TP={tp_adjustment:.2f}"
                        )
            ...
            if "sl_price" not in sl_tp_data:
                ...
                log.info(
                    f"SL/TP inicializado para {symbol}: SL @ {sl_price:.4f} (adj: {sl_adjustment:.2f}), TP @ {tp_price:.4f} (adj: {tp_adjustment:.2f})"
                )
            ...
            if position_side == "LONG":
                if current_price <= sl_price:
                    log.info(
                        f"🔴 Cierre por STOP LOSS: {symbol} @ {current_price} (SL: {sl_price})"
                    )
                    return "SL"
                elif current_price >= tp_price:
                    log.info(
                        f"🟢 Cierre por TAKE PROFIT: {symbol} @ {current_price} (TP: {tp_price})"
                    )
                    return "TP"
            else:
                if current_price >= sl_price:
    ...
    def check_balance_risk(self, account_info):
        ...
        if current_balance < config.MIN_BALANCE_THRESHOLD:
            log.warning(
                f"⚠️ Balance bajo: {current_balance:.2f} USDT (mínimo: {config.MIN_BALANCE_THRESHOLD} USDT)"
            )
            return True
    ...
    def analyze_trading_performance(self, symbol: str):
        ...
        def analyze_operation():
            ...
            try:
                ...
                log.info(
                    f"📊 Análisis de rendimiento para {symbol}: WR: {win_rate:.2%}, PF: {profit_factor:.2f}, Lev. Rec: {recommended_leverage}x, Vol: {atr:.2f}%, Dur: {avg_trade_duration:.1f}m"
                )
            ...
            try:
                ...
                for symbol, loss_count in recent_losing_trades:
                    ...
                    if total_trades > strategy_optimizer.MIN_TRADES_FOR_ANALYSIS:
                        ...
                        if loss_ratio > 0.7:
                            log.warning(
                                f"⚠️ Símbolo problemático detectado: {symbol} con {loss_ratio:.2%} de trades perdedores"
                            )
            except Exception as e:
        ...
        try:
            ...
            for symbol_data in problematic_symbols:
                ...
                if symbol_data["win_rate"] < 0.3:
                    ...
                    log.info(
                        f"🔒 Deshabilitando trading para {symbol} debido a mal performance"
                    )
                else:
                    config.SYMBOL_SPECIFIC_SETTINGS[symbol] = {
                        "risk_multiplier": 0.5,
                    ...
                    log.info(
                        f"⚠️ Reduciendo riesgo para {symbol} debido a performance mediocre"
                    )
...
"""
                ...
                log.info(
                    f"📊 Reporte diario generado: {total_trades} trades, P&L Diario: {total_pnl:+.2f} USDT (20x)"
                )
    ...
    def cleanup_invalid_symbols(self):
        ...
        with state_lock:
            invalid_trailing = [
                s for s in app_state["trailing_stop_data"] if s not in valid_symbols
            ]
            for symbol in invalid_trailing:
                del app_state["trailing_stop_data"][symbol]

            invalid_sltp = [
                s for s in app_state["sl_tp_data"] if s not in valid_symbols
            ]
            ...
            if invalid_trailing or invalid_sltp:
                log.info(
                    f"🧹 Limpiados {len(invalid_trailing + invalid_sltp)} símbolos inválidos"
                )

    def open_trade(self, symbol: str, side: str, last_candle):
        if not self.api.is_symbol_tradable(symbol):
            log.warning(
                f"⏭️ Símbolo {symbol} no está disponible para trading, omitiendo"
            )
            return
        ...
        if ticker:
            ...
            if bid_price > 0 and ask_price > 0:
                ...
                if spread > config.MAX_SPREAD_PERCENT:
                    log.warning(
                        f"⏭️ Spread demasiado alto ({spread:.2f}%) en {symbol}, omitiendo trade"
                    )
                    return
        ...
        if min_position_size * price > self.capital_manager.current_balance:
            log.info(
                f"⏭️ {symbol} requiere mínimo {min_position_size * price:.2f} USDT, capital insuficiente"
            )
            return
        ...
        if quantity < min_qty:
            log.info(
                f"⏭️ Cantidad {quantity} para {symbol} es menor que el mínimo {min_qty}"
            )
            return
        ...
        if notional_value < min_notional:
            log.info(
                f"⏭️ Valor nocional {notional_value:.2f} para {symbol} es menor que el mínimo {min_notional:.2f}"
            )
            return
        ...
        log.info(
            f"Attempting to place MARKET order for {side} {symbol} (Qty: {quantity}, Value: {notional_value:.2f} USDT, 20x)"
        )
        ...
        if order and order.get("orderId"):
            ...
            log.error(
                f"❌ Could not create market order for {symbol}. Response: {order}"
            )
    ...
    def run(self):
        ...
        log.info(
            f"🤖 Telegram: {'✅ CONFIGURADO' if self.telegram_notifier.enabled else '❌ NO CONFIGURADO'}"
        )
            ...
            try:
                ...
                with state_lock:
                    ...
                    if consecutive_losses >= config.MAX_CONSECUTIVE_LOSSES:
                        log.warning(
                            f"⛔ {consecutive_losses} pérdidas consecutivas, pausando trading por 1 hora"
                        )
                        time.sleep(3600)
                        continue
                ...
                if self.daily_pnl < -(
                    ...
                    log.warning(
                        f"⛔ Límite de pérdida diaria alcanzado: {self.daily_pnl:.2f} USDT (20x)"
                    )
                    if self.telegram_notifier.enabled:
                        self.telegram_notifier.notify_warning(
...
def api_export_logs():
    ...
    try:
        ...
        if os.path.exists(log_file_path):
            return send_file(
                log_file_path, as_attachment=True, download_name="trading_bot_logs.txt"
            )
