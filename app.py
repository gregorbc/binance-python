...
from database import PerformanceMetrics, SessionLocal, Trade
...
class CONFIG:
    ...
    # % de ganancia para activar trailing stop
    TRAILING_STOP_ACTIVATION: float = 0.9
    ...
    # Balance monitoring
    # Minimum USDT balance before stopping trades
...
class BinanceFutures:
    def __init__(self):
        ...
        if not api_key or not api_secret:
            raise ValueError(
                "API keys not configured. Set BINANCE_API_KEY and BINANCE_API_SECRET environment variables"
            )
    ...
    def ensure_symbol_settings(self, symbol: str):
        ...
        try:
            ...
            log.error(
                f"An unexpected error occurred setting margin type for {symbol}: {e}"
            )
    ...
    def _safe_api_call(self, func, *args, **kwargs):
        for attempt in range(3):
            try:
                ...
                if e.code == -4131:
                    log.warning(
                        "PERCENT_PRICE error (-4131) in order. Volatile or illiquid market. Skipping."
                    )
                    return None
...
class TradingBot:
    ...
    def get_top_symbols(self) -> List[str]:
        ...
        sorted_tickers = sorted(
            valid_tickers, key=lambda x: float(x["quoteVolume"]), reverse=True
        )
        ...
        with state_lock:
            ...
            if position_side == "LONG":
                ...
                if (
                    ...
                    and profit_percentage >= config.TRAILING_STOP_ACTIVATION
                ):
                    trailing_data["activated"] = True
                    trailing_data["current_stop"] = trailing_data["best_price"] * (
                        1 - config.TRAILING_STOP_PERCENTAGE / 100
                    )
                    log.info(
                        f"🔔 Trailing stop activado para {symbol} @ {trailing_data['current_stop']}"
                    )
                ...
                if trailing_data["activated"]:
                    ...
                    if new_stop > trailing_data["current_stop"]:
                        ...
                        log.info(
                            f"🔄 Trailing stop actualizado para {symbol}: {new_stop}"
                        )

                if (
                    trailing_data["activated"]
                    and current_price <= trailing_data["current_stop"]
                ):
                    log.info(
    ...
    def check_fixed_sl_tp(self, symbol: str, position: Dict, current_price: float):
        ...
        with state_lock:
            ...
            if "sl_price" not in sl_tp_data:
                ...
                log.info(
                    f"Initialized SL/TP for {symbol}: SL @ {sl_price:.4f}, TP @ {tp_price:.4f}"
                )
            ...
            if not sl_price or not tp_price:
                log.warning(
                    f"SL/TP prices not found for {symbol} after initialization attempt."
                )
                return False
    ...
    def check_balance_risk(self, account_info):
        ...
        usdt_balance = next(
            (
                float(a.get("walletBalance", 0) or 0)
        ...
        if usdt_balance < config.MIN_BALANCE_THRESHOLD:
            log.warning(
                f"⚠️ Balance bajo: {usdt_balance} USDT (mínimo: {config.MIN_BALANCE_THRESHOLD} USDT)"
            )
            return True
    ...
    def analyze_trading_performance(self, symbol: str):
        try:
            ...
            log.info(
                f"📊 Análisis de rendimiento para {symbol}: "
    ...
    def optimize_strategy_based_on_losses(self):
        try:
            ...
            symbol_loss_count = {
                symbol: losing_symbols.count(symbol) for symbol in set(losing_symbols)
            }
            ...
            problem_symbols = {
                ...
                if total_trades_by_symbol[symbol] > 0
                and loss_count / total_trades_by_symbol[symbol] > 0.7
            }
            ...
            try:
                ...
                for symbol, position in open_positions.items():
                    try:
                        ...
                        should_close_trailing = self.check_trailing_stop(
                            symbol, position, current_price
                        )
                        sl_tp_signal = self.check_fixed_sl_tp(
                            symbol, position, current_price
                        )
                        should_close = should_close_trailing or (
                            sl_tp_signal in ["SL", "TP"]
                        )
                        ...
                        if should_close and not config.DRY_RUN:
                            ...
                            if result:
                                pnl_records = self.api._safe_api_call(
                                    self.api.client.futures_account_trades,
                                    symbol=symbol,
                                    limit=10,
                                )
                                realized_pnl = 0.0
                                if pnl_records:
                                    realized_pnl = sum(
                                        ...
                                        if trade.get("orderId") == result.get("orderId")
                                    )

                                close_type = (
                                    "trailing_stop"
                                    if should_close_trailing
                                    else sl_tp_signal.lower()
                                )
                                ...
                                log.info(
                                    f"✅ Position closed: {symbol}, PnL: {realized_pnl:.2f} USDT, Reason: {close_type}"
                                )

                    except Exception as e:
                        log.error(
                            f"Error checking stops for {symbol}: {e}", exc_info=True
                        )
                ...
                if num_open_pos < config.MAX_CONCURRENT_POS and not low_balance:
                    ...
                    log.info(
                        f"🔍 Scanning {len(symbols_to_scan)} symbols for new signals."
                    )
                ...
                with state_lock:
                    ...
                    if stats["trades_count"] > 0:
                        stats["win_rate"] = (
                            stats["wins"] / stats["trades_count"]
                        ) * 100
                        winning_trades = [
                            t
                            for t in app_state["trades_history"]
                            if t.get("pnl", 0) > 0
                        ]
                        stats["avg_win"] = (
    ...
    def open_trade(self, symbol: str, side: str, last_candle):
        ...
        if order and order.get("orderId"):
            log.info(
                f"✅ LIMIT ORDER CREATED: {side} {quantity} {symbol} @ {limit_price}"
            )
            ...
            log.error(
                f"❌ Could not create limit order for {symbol}. Response: {order}"
            )
...
def apply_exchange_trailing_stop(binance_api: BinanceFutures, symbol: str):
    ...
    try:
        ...
        pos = next(
            (p for p in acct.get("positions", []) if p["symbol"] == symbol), None
        )
        ...
        if config.DRY_RUN:
            ...
            socketio.emit(
                "trailing_update",
                {"symbol": symbol, "stop_price": stop_price, "dry_run": True},
            )
            return

        result = binance_api._safe_api_call(
            binance_api.client.futures_create_order, **params
        )
        ...
        if result:
            ...
            log.info(
                f"🛡️ STOP_MARKET placed for {symbol} @ {stop_price} (orderId={result.get('orderId')})"
            )
            socketio.emit(
                "trailing_update",
                {
            ...
            log.warning(
                f"❌ Failed to place STOP_MARKET for {symbol} @ {stop_price} (result={result})"
            )

    except Exception as e:
        log.error(
            f"Error placing exchange trailing stop for {symbol}: {e}", exc_info=True
        )
    ...
    while True:
        try:
            ...
            for symbol in symbols_to_check:
                try:
                    mark = binance_api._safe_api_call(
                        binance_api.client.futures_mark_price, symbol=symbol
                    )
                    if not mark:
                        continue
                    ...
                    if not trailing_data:
                        if acct:
                            ...
                            if pos and float(pos.get("positionAmt", 0)) != 0:
                                entry_price = float(
                                    pos.get("entryPrice", current_price)
                                    or current_price
                                )
                                side = (
                                ...
                                trailing_data = app_state["trailing_stop_data"].get(
                                    symbol
                                )
                    ...
                    log.error(
                        f"Error in monitor loop for {symbol}: {inner_e}", exc_info=True
                    )
        ...
        try:  # Fallback to check directly with Binance
            ...
            return (
                jsonify(
                    {
                        "status": "error",
                        "message": f"No cached position and API check failed for {symbol}",
                    }
                ),
                404,
            )
    ...
    try:
        ...
        if result:
            pnl_records = api._safe_api_call(
                api.client.futures_account_trades, symbol=symbol, limit=10
            )
            realized_pnl = 0.0
            if pnl_records:
                ...
                if order_id_to_match:
                    realized_pnl = sum(
                        float(trade.get("realizedPnl", 0))
                        for trade in pnl_records
                        if str(trade.get("orderId")) == str(order_id_to_match)
                    )

            mark = api._safe_api_call(api.client.futures_mark_price, symbol=symbol)
            mark_price = (
                float(mark["markPrice"]) if mark else float(position["entryPrice"])
            )

            entry_price = float(position["entryPrice"])
            position_size = abs(float(position["positionAmt"]))
            roe = (
                (realized_pnl / (position_size * entry_price)) * 100 * config.LEVERAGE
                if entry_price
                else 0.0
            )
            ...
            log.info(
                f"✅ Position on {symbol} closed. Realized PNL: {realized_pnl:.2f} USDT, ROE: {roe:.2f}%"
            )
            socketio.emit("status_update", app_state)
            return jsonify(
...
def get_trade_history():
    ...
    if symbol_filter:
        filtered_trades = [
            t for t in filtered_trades if t.get("symbol") == symbol_filter
        ]
    if date_filter:
        try:
            ...
            filtered_trades = [
                ...
                if _dt.fromtimestamp(t.get("timestamp", 0)).date() == filter_date
            ]
        except Exception:
            pass
    if side_filter:
...
def get_performance_metrics():
    try:
        ...
        if hours:
            query = query.filter(
                PerformanceMetrics.timestamp >= datetime.now() - timedelta(hours=hours)
            )
