...
class CONFIG:
    ...
    ATR_MULT_TP: float = (
        4.0  # Multiplicador de ATR para Take Profit (o usar porcentaje)
    )
...
class MarketAnalyzer:
    ...
    def analyze_market_conditions(
        ...
        plus_di = talib.PLUS_DI(df["high"], df["low"], df["close"], timeperiod=14).iloc[
            -1
        ]
...
class BinanceFutures:
    ...
    def ensure_symbol_settings(self, symbol: str):
        ...
        try:
            ...
            if not (e.code == -4046 or "No need to change margin type" in e.message):
...
class TradingBot:
    ...
    def calculate_signal_strength(
        ...
        price_distance = (
            abs(df["fast_ema"].iloc[-1] - df["slow_ema"].iloc[-1])
            / df["close"].iloc[-1]
        )
        macd_strength = (
            0.8
            if (signal == "LONG" and df["macd"].iloc[-1] > df["macd_signal"].iloc[-1])
            or (signal == "SHORT" and df["macd"].iloc[-1] < df["macd_signal"].iloc[-1])
            else 0.5
        )
        ...
        bb_strength = (
            ...
            or (signal == "SHORT" and bb_position > 0.8)
            else 0.5
        )
        market_conditions = self.market_analyzer.analyze_market_conditions(symbol, df)
        trend_alignment = 0.5
        if (market_conditions["market_regime"] == 1 and signal == "LONG") or (
            market_conditions["market_regime"] == 2 and signal == "SHORT"
        ):
            trend_alignment = 0.9
            ...
            trend_alignment = 0.2
        regime_factor = 1.0
        if (self.market_regime == "BULL" and signal == "LONG") or (
            self.market_regime == "BEAR" and signal == "SHORT"
        ):
            regime_factor = 1.2
    ...
    def check_signal(self, df: pd.DataFrame, symbol: str) -> Optional[str]:
        ...
        if not signal:
            return None
        rsi_confirm = (signal == "LONG" and last["rsi"] > 45) or (
            signal == "SHORT" and last["rsi"] < 55
        )
        ...
        if strength < config.MIN_SIGNAL_STRENGTH:
            log.info(
                f"📉 Señal {signal} en {symbol} descartada por fuerza insuficiente: {strength:.2f}"
            )
            return None
    ...
    def get_ai_adjustments(
        ...
        if performance_data:
            state_key += f"_WR{int(performance_data.get('win_rate', 0.5) * 100)}_PF{performance_data.get('profit_factor', 1.0):.1f}"
    ...
    def update_ai_model(
        ...
        try:
            ...
            if exit_reason == "tp":
                ...
                reward = 1.5 + (
                    trade_result / (abs(trade_result) + 1.0)
                )  # Recompensa alta por dejar correr ganancias
            elif trade_result > 0:
                reward = 1.0 * (
                    trade_result / (abs(trade_result) + 1.0)
                )  # Recompensa moderada
            ...
            log.info(
                f"🤖 AI model updated for {symbol} with reward {reward:.2f} for exit type '{exit_reason}'"
            )
            ...
            if (config.AI_SL_ADJUSTMENT or config.AI_TP_ADJUSTMENT) and DB_ENABLED:
                ...
                if performance_data:
                    ...
                    if sl_adjustment != 1.0 or tp_adjustment != 1.0:
                        log.info(
                            f"🤖 AI SL/TP Adjustment for {symbol}: SL={sl_adjustment:.2f}, TP={tp_adjustment:.2f}"
                        )

            if "sl_price" not in sl_tp_data:
                # Lógica de SL basada en ATR
                atr = talib.ATR(df["high"], df["low"], df["close"], timeperiod=14).iloc[
                    -1
                ]
                ...
                log.info(
                    f"Initialized ATR-based SL/TP for {symbol}: SL @ {sl_price:.4f}, TP @ {tp_price:.4f}"
                )
    ...
    def dynamic_exit_strategy(
        ...
        if self.check_trailing_stop(symbol, position, current_price):
            return "TRAILING"
        if self.check_time_based_exit(symbol, position):
            return "TIME_EXIT"
        if self.check_momentum_reversal(symbol, position, df):
                    ...
                    try:
                        ...
                        if exit_signal:
                            ...
                            if close_order:
                                sl_tp_data = app_state["sl_tp_data"].get(symbol, {})
                                sl_adj, tp_adj = (
                                    sl_tp_data.get("sl_adjustment", 1.0),
                                    sl_tp_data.get("tp_adjustment", 1.0),
                                )
    ...
    def open_trade(self, symbol: str, side: str, last_candle):
        ...
        limit_price = self.api.round_value(
            (
                price + filters["tickSize"] * 5
                if side == "LONG"
                else price - filters["tickSize"] * 5
            ),
            filters["tickSize"],
        )
...
def _record_closed_trade(
    ...
    try:
        ...
        entry_price, position_size = (
            float(position_data["entryPrice"]),
            abs(float(position_data["positionAmt"])),
        )
        roe = (
            (realized_pnl / (position_size * entry_price / config.LEVERAGE)) * 100
            if entry_price > 0 and position_size > 0
            else 0.0
        )

        entry_time = datetime.fromtimestamp(
            int(position_data.get("updateTime", 0)) / 1000
        )
        ...
        log.info(
            f"✅ Position closed: {symbol}, PnL: {realized_pnl:.2f} USDT, ROE: {roe:.2f}%, Reason: {close_type}"
        )
        socketio.emit("status_update", app_state)
        return realized_pnl
    except Exception as e:
...
def close_position_api():
    ...
    try:
        ...
        if not position:
            return (
                jsonify(
                    {"status": "error", "message": f"No active position for {symbol}"}
                ),
                404,
            )
