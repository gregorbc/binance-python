...
try:
    ...
    DRL_ENABLED = False
    print(
        "ADVERTENCIA: TensorFlow no encontrado. Las características de Deep Learning estarán deshabilitadas. Por gregorbc@gmail.com"
    )
...
try:
    ...
    DB_ENABLED = False
    print(
        f"ADVERTENCIA: 'database.py' no encontrado. Las características de base de datos estarán deshabilitadas. Error: {e}"
    )
...
class CONFIG:
    ...
    # **Mejora:** Reducido a 1.0 para 20x leverage para una gestión de riesgo más estricta.
    ...
    BLACKLIST_SYMBOLS: tuple = (
        ...
        "NEARUSDT",
        "FTMUSDT",
    )  # **Mejora:** Añadidos más pares para reducir el riesgo.
    TIMEFRAME: str = "5m"
    ...
    RISK_PER_TRADE_PERCENT: float = (
        # **Mejora:** Reducido a 1.5% para una gestión de riesgo más estricta.
        1.5
    )
    ...
    MAX_POSITION_HOLD_HOURS: int = (
        2  # **Mejora:** Reducido para 20x para evitar posiciones estancadas.
    )
    ...
    SYMBOL_SPECIFIC_SETTINGS: Dict = field(
    ...
    # Umbral de volumen bajo (30% del promedio)
    ...
    def __post_init__(self):
        ...
        for config_name in required_configs:
            if not hasattr(self, config_name):
                raise ValueError(
                    f"❌ Configuración requerida faltante: {config_name}")
...
class CapitalManager:
    ...
    def get_real_balance(self) -> float:
        ...
        try:
            account_info = self.api._safe_api_call(
            ...
            log.error(
                f"Error obteniendo balance real: {e} Por gregorbc@gmail.com")
        return 0.0
    ...
    def update_balance(self, force: bool = False) -> bool:
        ...
        if force or (
            current_time - self.last_balance_update > 240
        ):  # Reducido a 4 min para 20x
            new_balance = self.get_real_balance()
            if new_balance > 0:
                if self.current_balance == 0:
                    self.initial_balance = new_balance
                    log.info(
                        f"💰 Capital inicial: {new_balance:.2f} USDT (20x leverage) Por gregorbc@gmail.com"
                    )
        ...
        def save_operation():
            ...
            if db:
                try:
                    ...
                    db.commit()
                    log.info(
                        f"💾 Balance guardado en DB: {self.current_balance:.2f} USDT"
                    )
                except Exception as e:
                    log.error(
                        f"Error guardando balance en DB: {e} Por gregorbc@gmail.com"
                    )
                    db.rollback()
                finally:
                    db.close()
    ...
    def reinvest_profit(self, profit: float):
        ...
        if profit > 0 and self.should_reinvest(profit):
            ...
            log.info(
                f"🔄 Reinvirtiendo {profit:.2f} USDT en el capital (20x) Por gregorbc@gmail.com"
            )
    ...
    def update_model(self, state: str, reward: float, sl_adj: float, tp_adj: float):
        ...
        if state in self.q_table:
            ...
            new_reward = current_reward + \
                self.learning_rate * (reward - current_reward)
    ...
    def _load_performance_data(self):
        ...
        def load_operation():
            ...
            try:
                ...
                log.error(
                    f"Error cargando datos de rendimiento: {e} Por gregorbc@gmail.com"
                )
            finally:
                db.close()
...
class TelegramNotifier:
    ...
    def __init__(self):
        ...
        if self.enabled:
            ...
            log.warning(
                "⚠️ Bot de Telegram no configurado. Agrega TELEGRAM_BOT_TOKEN y TELEGRAM_CHAT_ID en .env"
            )
...
⏰ {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}
    ...
    def notify_trade_closed(
        ...
        emoji = "🟢" if pnl >= 0 else "🔴"
        roe = (
            (pnl / (quantity * entry_price / leverage)) *
...
⏰ {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}
    ...
    def notify_reinvestment(
        self, amount: float, new_balance: float, leverage: int = 20
    ):
...
📉 <b>Avg Loss:</b> ${stats["avg_loss"]:.2f}
    ...
    def send_performance(self, chat_id: int):
        ...
        try:
            ...
            if not db:
                self.send_message(
                    chat_id, "❌ Error de conexión a base de datos")
                return
            ...
            for stat in symbol_stats:
                win_rate = (
                    (stat.winning_trades / stat.total_trades * 100)
...
📊 <b>Avg P&L:</b> ${stat.avg_pnl:+.2f}
            ...
            self.send_message(
                chat_id, f"❌ Error obteniendo estadísticas: {str(e)}")
        finally:
            if "db" in locals():
                db.close()
...
class TelegramPoller:
    ...
    def start_polling(self):
        ...
        if not self.token:
            log.warning(
                "❌ No se puede iniciar polling - Token de Telegram no configurado"
            )
            return
...
if not log.handlers:
    ...
    file_handler = logging.FileHandler(
        f"logs/{config.LOG_FILE}", encoding="utf-8", mode="a"
    )
...
app_state = {
    ...
    "connection_metrics": {"api_retries": 0, "last_reconnect": None, "uptime": 0.0},
    "ai_metrics": {
        "q_table_size": 0,
...
class BinanceFutures:
    def __init__(self):
        ...
        if not api_key or not api_secret:
            raise ValueError(
                "API keys no configuradas. Establezca las variables de entorno BINANCE_API_KEY and BINANCE_API_SECRET Por gregorbc@gmail.com"
            )

        self.client = Client(api_key, api_secret, testnet=testnet)
        log.info(
            f"🔧 CONECTADO A BINANCE FUTURES {'TESTNET' if testnet else 'MAINNET'} - 20x LEVERAGE Por gregorbc@gmail.com"
        )

        try:
            self.exchange_info = self.client.futures_exchange_info()
            log.info(
                "✅ Información de exchange cargada exitosamente Por gregorbc@gmail.com"
            )
        except Exception as e:
            log.error(
                f"❌ Error conectando a Binance: {e} Por gregorbc@gmail.com")
            raise

    def ensure_symbol_settings(self, symbol: str):
        try:
            _ = self._safe_api_call(
                self.client.futures_change_leverage,
                symbol=symbol,
                leverage=int(config.LEVERAGE),
            )
            log.info(
                f"✅ Apalancamiento configurado a {config.LEVERAGE}x para {symbol} Por gregorbc@gmail.com"
            )
        except Exception as e:
            log.warning(
                f"Problema al establecer leverage para {symbol}: {e} Por gregorbc@gmail.com"
            )
        ...
        try:
            ...
            if e.code == -4046 or "No need to change margin type" in e.message:
                ...
                log.warning(
                    f"Advertencia al establecer tipo de margen para {symbol}: {e} Por gregorbc@gmail.com"
                )
        except Exception as e:
            log.error(
                f"Error inesperado al establecer tipo de margen para {symbol}: {e} Por gregorbc@gmail.com"
            )
    ...
    def _safe_api_call(self, func, *args, **kwargs):
        for attempt in range(config.MAX_API_RETRIES):
            try:
                ...
                if e.code == -4131:
                    log.warning(
                        "Error PERCENT_PRICE (-4131) en orden. Mercado volátil or ilíquido. Omitiendo. Por gregorbc@gmail.com"
                    )
                    return None
                elif e.code in [-2011, -2021]:
                    log.warning(
                        f"Error de API en orden ({e.code}): {e.message} Por gregorbc@gmail.com"
                    )
                else:
                    log.warning(
                        f"Error no crítico de API: {e.code} - {e.message} Por gregorbc@gmail.com"
                    )
                    if attempt == config.MAX_API_RETRIES - 1:
                        log.error(
                            f"Error final de API después de todos los reintentos: {e.code} - {e.message} Por gregorbc@gmail.com"
                        )
            except Exception as e:
                with state_lock:
                    app_state["connection_metrics"]["api_retries"] += 1
                log.warning(
                    f"Error general en llamada a API: {e} Por gregorbc@gmail.com"
                )
                if attempt == config.MAX_API_RETRIES - 1:
                    log.error(
                        f"Error general final después de todos los reintentos: {e} Por gregorbc@gmail.com"
                    )
    ...
    def smart_retry_api_call(self, func, *args, **kwargs):
        ...
        for attempt, delay in enumerate(retry_delays):
            try:
                ...
                if attempt > 0:
                    log.info(
                        f"✅ Reintento exitoso después de {attempt} intentos Por gregorbc@gmail.com"
                    )
                ...
                if e.code in [-1013, -2010, -2011]:
                    log.error(
                        f"Error no recuperable: {e.message} (código: {e.code}) Por gregorbc@gmail.com"
                    )
                    break

                log.warning(
                    f"Reintentando en {delay}s (intento {attempt + 1}/{len(retry_delays)}) Por gregorbc@gmail.com"
                )
                time.sleep(delay)
                ...
                last_exception = e
                log.warning(
                    f"Error general, reintentando en {delay}s (intento {attempt + 1}/{len(retry_delays)}) Por gregorbc@gmail.com"
                )
                time.sleep(delay)

        log.error(
            f"❌ Todos los reintentos fallados: {last_exception} Por gregorbc@gmail.com"
        )
        return None
    ...
    def place_order(
        ...
        if order_type == FUTURE_ORDER_TYPE_LIMIT:
            if price is None:
                log.error(
                    "Precio requerido para órdenes LIMIT. Por gregorbc@gmail.com")
                return None
            params.update(
                {"price": str(price), "timeInForce": TIME_IN_FORCE_GTC})
    ...
    def close_position(self, symbol: str, position_amt: float) -> Optional[Dict]:
        ...
        if config.DRY_RUN:
            log.info(
                f"[DRY_RUN] close_position {symbol} {position_amt} Por gregorbc@gmail.com"
            )
            return {"mock": True, "orderId": int(time.time() * 1000)}
    ...
    def cancel_order(self, symbol: str, orderId: int) -> Optional[Dict]:
        if config.DRY_RUN:
            log.info(
                f"[DRY_RUN] cancel_order: {orderId} para {symbol} Por gregorbc@gmail.com"
            )
            return {"mock": True}

        try:
            return self.smart_retry_api_call(
                self.client.futures_cancel_order, symbol=symbol, orderId=orderId
            )
        except Exception as e:
            log.warning(
                f"No se pudo cancelar la orden {orderId} para {symbol}: {e} Por gregorbc@gmail.com"
            )
            return None
...
class TradingBot:
    def __init__(self):
        ...
        try:
            self.performance_analyzer = PerformanceAnalyzer(
                SessionLocal() if DB_ENABLED else None
            )
        except Exception as e:
    ...
    def get_top_symbols(self) -> List[str]:
        ...
        for t in valid_tickers:
            ...
            min_notional = max(
                filters.get("minNotional", 5.0), config.MIN_NOTIONAL_OVERRIDE
            )
        ...
        suitable_symbols.sort(
            key=lambda x: (float(x["quoteVolume"]), 1 / float(x["lastPrice"])),
            reverse=True,
        )
    ...
    def calculate_indicators(self, df: pd.DataFrame):
        ...
        df["fast_ema"] = df["close"].ewm(
            span=config.FAST_EMA, adjust=False).mean()
        df["slow_ema"] = df["close"].ewm(
            span=config.SLOW_EMA, adjust=False).mean()
        ...
        stoch_rsi = (df["rsi"] - df["rsi"].rolling(config.STOCH_PERIOD).min()) / (
            df["rsi"].rolling(config.STOCH_PERIOD).max()
            - df["rsi"].rolling(config.STOCH_PERIOD).min()
        )
    ...
    def calculate_adx(self, df: pd.DataFrame, period: int = 14) -> float:
        ...
        try:
            ...
            log.error(
                f"Error calculando ADX para {symbol}: {e} Por gregorbc@gmail.com")
            return 0.0
    ...
    def _calculate_adx_manual(self, df: pd.DataFrame, period: int = 14) -> float:
        ...
        try:
            ...
            plus_dm = np.where((up_move > down_move) &
                               (up_move > 0), up_move, 0)
            minus_dm = np.where(
                (down_move > up_move) & (down_move > 0), down_move.abs(), 0
            )
            ...
            log.error(
                f"Error en cálculo manual de ADX: {e} Por gregorbc@gmail.com")
            return 0.0

    def calculate_signal_strength(
        self, df: pd.DataFrame, signal: str, symbol: str
    ) -> float:
        ...
        volume_ratio = (
            min(2.0, df["volume"].iloc[-1] /
                volume_avg) if volume_avg > 0 else 1
        )
        ...
        price_distance = (
            ema_distance /
            df["close"].iloc[-1] if df["close"].iloc[-1] > 0 else 0
        )
    ...
    def check_signal(self, df: pd.DataFrame, symbol: str) -> Optional[str]:
        ...
        if len(df) >= 28:
            ...
            if adx < config.MIN_ADX_FOR_TREND:
                log.info(
                    f"↔️ Mercado lateral para {symbol} (ADX: {adx:.1f} < {config.MIN_ADX_FOR_TREND}), omitiendo Por gregorbc@gmail.com"
                )
                return None
        ...
        if "volatility" in df.columns and not pd.isna(df["volatility"].iloc[-1]):
            ...
            if volatility > config.MAX_VOLATILITY_PERCENT:
                log.info(
                    f"⚡ Alta volatilidad en {symbol}: {volatility:.1f}%, omitiendo Por gregorbc@gmail.com"
                )
                return None
        ...
        if config.VOLUME_WEIGHTED_SIGNALS:
            ...
            if volume_ratio < config.MIN_VOLUME_CONFIRMATION:
                log.info(
                    f"📉 Señal {signal} descartada por volumen insuficiente: {volume_ratio:.2f} Por gregorbc@gmail.com"
                )
                return None

        if config.HIGHER_TIMEFRAME_CONFIRMATION:
            try:
                df_higher = self.get_klines_for_symbol(
                    symbol, interval=config.HIGHER_TIMEFRAME, limit=50
                )
                if df_higher is not None and len(df_higher) > 20:
                    ...
                    if (
                        last_higher["fast_ema"] > last_higher["slow_ema"]
                        and prev_higher["fast_ema"] <= prev_higher["slow_ema"]
                    ):
                    ...
                    if higher_tf_signal != signal:
                        log.info(
                            f"⏭️ Señal {signal} en {symbol} descartada por falta de alineación con TF superior ({higher_tf_signal}) Por gregorbc@gmail.com"
                        )
                        return None
            except Exception as e:
                log.error(
                    f"Error verificando TF superior para {symbol}: {e} Por gregorbc@gmail.com"
                )

        if config.BTC_CORRELATION_FILTER and symbol != "BTCUSDT":
            btc_df = self.get_klines_for_symbol(
                "BTCUSDT", interval=config.TIMEFRAME, limit=50
            )
            if btc_df is not None and len(btc_df) > 20:
                ...
                if (
                    last_btc["fast_ema"] > last_btc["slow_ema"]
                    and prev_btc["fast_ema"] <= prev_btc["slow_ema"]
                ):
                    btc_signal = "LONG"
                elif (
                    last_btc["fast_ema"] < last_btc["slow_ema"]
                ...
                if btc_signal and btc_signal != signal:
                    strength = self.calculate_signal_strength(
                        df, signal, symbol
                    )  # Recalculate strength before reducing
                    strength *= 0.6
                    log.info(
                        f"⚠️ Señal {signal} en {symbol} con señal contraria en BTC, fuerza reducida a {strength:.2f} Por gregorbc@gmail.com"
                    )
        ...
        if strength < config.MIN_SIGNAL_STRENGTH:
            log.info(
                f"📉 Señal {signal} descartada por fuerza insuficiente: {strength:.2f} Por gregorbc@gmail.com"
            )
            return None

        log.info(
            f"📶 Señal {signal} con fuerza: {strength:.2f}, confirmaciones: {confirmations}/3 Por gregorbc@gmail.com"
        )
        return signal

    def get_ai_adjustments(
        self, symbol: str, performance_data: Dict
    ) -> Tuple[float, float]:
        """Obtiene ajustes de SL/TP de la IA Por gregorbc@gmail.com"""
        market_conditions = self.market_analyzer.analyze_market_conditions(
            symbol, self.get_klines_for_symbol(symbol)
        )

        volatility_state = (
            "HIGH"
            if market_conditions["volatility"]
            > self.market_analyzer.volatility_threshold
            else "LOW"
        )
    ...
    def update_ai_model(
        ...
        try:
            ...
            trend_state = (
                "TREND" if market_conditions["trend_strength"] > 25 else "RANGE"
            )
            market_state = (
                "BULL"
                if self.market_regime == "BULL"
            ...
            state_key = (
                f"{volatility_state}_{trend_state}_{market_state}_EXIT_{exit_reason}"
            )
            ...
            with state_lock:
                app_state["ai_metrics"]["q_table_size"] = len(
                    self.ai_model.q_table)
                app_state["ai_metrics"]["last_learning_update"] = (
                    datetime.now().isoformat()
                )
                app_state["ai_metrics"]["exploration_rate"] = (
                    self.ai_model.exploration_rate
                )
        except Exception as e:
            log.error(
                f"Error actualizando modelo de IA para {symbol}: {e} Por gregorbc@gmail.com"
            )

    def check_trailing_stop(self, symbol: str, position: Dict, current_price: float):
        with state_lock:
            position_side = "LONG" if float(
                position["positionAmt"]) > 0 else "SHORT"
            entry_price = float(position["entryPrice"])
            ...
            if config.AI_TRAILING_ADJUSTMENT and DB_ENABLED:
                ...
                if performance_data:
                    volatility_factor = (
                        performance_data.get("market_volatility", 1.0)
                        * config.AI_VOLATILITY_FACTOR
                    )
                    trailing_adjustment = max(0.7, min(1.5, volatility_factor))
                    if trailing_adjustment != 1.0:
                        log.info(
                            f"🤖 Ajuste de Trailing por IA para {symbol}: {trailing_adjustment:.2f} (Volatilidad: {performance_data.get('market_volatility', 0):.2f}%) Por gregorbc@gmail.com"
                        )
            ...
            activation_percent = (
                ...
                trailing_data["trailing_adjustment"]
            )
            stop_percentage = (
            ...
            if position_side == "LONG":
                ...
                if (
                    not trailing_data["activated"]
                    and profit_percentage >= activation_percent
                ):
                    trailing_data["activated"] = True
                    log.info(
                        f"🔔 Trailing stop activado para {symbol} (adj: {trailing_data['trailing_adjustment']:.2f}) Por gregorbc@gmail.com"
                    )
                if trailing_data["activated"]:
                    new_stop = trailing_data["best_price"] * \
                    ...
                    if current_price <= trailing_data["current_stop"]:
                        log.info(
                            f"🔴 Cierre por trailing stop: {symbol} @ {current_price} (Stop: {trailing_data['current_stop']}) Por gregorbc@gmail.com"
                        )
                        should_close = True
            else:
                if current_price < trailing_data["best_price"]:
    ...
    def check_fixed_sl_tp(self, symbol: str, position: Dict, current_price: float):
        ...
        with state_lock:
            ...
            if (config.AI_SL_ADJUSTMENT or config.AI_TP_ADJUSTMENT) and DB_ENABLED:
                performance_data = self.analyze_trading_performance(symbol)
                if performance_data:
                    sl_adj, tp_adj = self.get_ai_adjustments(
                    ...
                    if sl_adjustment != 1.0 or tp_adjustment != 1.0:
                        log.info(
                            f"🤖 Ajuste SL/TP por IA para {symbol}: SL={sl_adjustment:.2f}, TP={tp_adjustment:.2f} Por gregorbc@gmail.com"
                        )

            if "sl_price" not in sl_tp_data:
                if position_side == "LONG":
                    sl_price = entry_price * (
                        1 - (config.STOP_LOSS_PERCENT * sl_adjustment) / 100
                    )
                    tp_price = entry_price * (
                        1 + (config.TAKE_PROFIT_PERCENT * tp_adjustment) / 100
                    )
                else:
                    sl_price = entry_price * (
                        1 + (config.STOP_LOSS_PERCENT * sl_adjustment) / 100
                ...
                log.info(
                    f"SL/TP inicializado para {symbol}: SL @ {sl_price:.4f} (adj: {sl_adjustment:.2f}), TP @ {tp_price:.4f} (adj: {tp_adjustment:.2f}) Por gregorbc@gmail.com"
                )
            ...
            if position_side == "LONG":
                if current_price <= sl_price:
                    log.info(
                        f"🔴 Cierre por STOP LOSS: {symbol} @ {current_price} (SL: {sl_price}) Por gregorbc@gmail.com"
                    )
                    return "SL"
                elif current_price >= tp_price:
                    log.info(
                        f"🟢 Cierre por TAKE PROFIT: {symbol} @ {current_price} (TP: {tp_price}) Por gregorbc@gmail.com"
                    )
                    return "TP"
            else:
                if current_price >= sl_price:
    ...
    def check_time_based_exit(self, symbol: str, position: Dict) -> bool:
        ...
        if hours_held > config.MAX_POSITION_HOLD_HOURS:
            log.info(
                f"⏰ Cierre por tiempo: {symbol} held for {hours_held:.1f} hours Por gregorbc@gmail.com"
            )
            return True
        return False

    def check_momentum_reversal(
        self, symbol: str, position: Dict, df: pd.DataFrame
    ) -> bool:
        ...
        if position_side == "LONG":
            ...
            if price_trend and rsi_trend and current_rsi > 70:
                log.info(
                    f"↘️ Cierre por divergencia RSI bajista: {symbol} Por gregorbc@gmail.com"
                )
                return True
        else:
            price_trend = df["close"].iloc[-1] < df["close"].iloc[-3]
            ...
            if price_trend and rsi_trend and current_rsi < 30:
                log.info(
                    f"↗️ Cierre por divergencia RSI alcista: {symbol} Por gregorbc@gmail.com"
                )
                return True

        return False

    def dynamic_exit_strategy(
        self, symbol: str, position: Dict, current_price: float, df: pd.DataFrame
    ) -> Optional[str]:
        ...
        if df is not None and len(df) > 20:
            ...
            if current_volume < volume_avg * config.LOW_VOLUME_THRESHOLD:
                log.info(
                    f"⚠️ Volumen anormalmente bajo en {symbol}, cerrando posición Por gregorbc@gmail.com"
                )
        ...
        trailing_stop_signal = self.check_trailing_stop(
    ...
    def check_balance_risk(self, account_info):
        ...
        if current_balance < config.MIN_BALANCE_THRESHOLD:
            log.warning(
                f"⚠️ Balance bajo: {current_balance:.2f} USDT (mínimo: {config.MIN_BALANCE_THRESHOLD} USDT) Por gregorbc@gmail.com"
            )
            return True
        ...
        total_investment = sum(
            float(p.get("initialMargin", 0) or 0) for p in open_positions.values()
        )
        exposure_ratio = (
            total_investment / current_balance if current_balance > 0 else 0
        )

        if exposure_ratio > 0.8:
            log.warning(
                f"⚠️ Exposición demasiado alta: {exposure_ratio:.2%} Por gregorbc@gmail.com"
            )
        ...
        with state_lock:
            ...
            if app_state["balance_history"]:
                ...
                drawdown = (peak - current_balance) / \
                    peak * 100 if peak > 0 else 0
                app_state["risk_metrics"]["max_drawdown"] = max(
                    app_state["risk_metrics"]["max_drawdown"], drawdown
                )

                if drawdown > config.MAX_DRAWDOWN_PERCENT:
                    log.warning(
                        f"⛔ Drawdown máximo excedido: {drawdown:.2f}% Por gregorbc@gmail.com"
                    )
                    return True
    ...
    def calculate_position_size(self, symbol: str, price: float) -> float:
        ...
        if self.performance_analyzer:
            risk_factor = self.performance_analyzer.get_symbol_risk_factor(
                symbol)
        ...
        if filters:
            ...
            position_size = min(
                position_size, max_position_size * 0.8
            )  # Margen de seguridad
            ...
            try:
                recent_trades = (
                    ...
                    .filter(
                        ...
                        Trade.timestamp
                        >= datetime.now()
                        - timedelta(hours=strategy_optimizer.OPTIMIZATION_INTERVAL),
                    )
                    .all()
                )
                ...
                win_rate = (
                    len(winning_trades) /
                    len(recent_trades) if recent_trades else 0
                )
                avg_win = (
                ...
                trade_durations = [
                    t.duration for t in recent_trades if t.duration is not None
                ]
                avg_trade_duration = (
                ...
                if win_rate > 0.7 and profit_factor > 2.0:
                    recommended_leverage = min(
                        config.LEVERAGE + strategy_optimizer.LEVERAGE_ADJUSTMENT_STEP,
                ...
                if atr > strategy_optimizer.VOLATILITY_THRESHOLD:
                    recommended_leverage = max(
                        ...
                        - strategy_optimizer.LEVERAGE_ADJUSTMENT_STEP,
                        strategy_optimizer.MIN_LEVERAGE,
                    )
                ...
                log.info(
                    f"📊 Análisis de rendimiento para {symbol}: WR: {win_rate:.2%}, PF: {profit_factor:.2f}, Lev. Rec: {recommended_leverage}x, Vol: {atr:.2f}%, Dur: {avg_trade_duration:.1f}m Por gregorbc@gmail.com"
                )
                return asdict(metrics)

            except Exception as e:
                log.error(
                    f"Error analizando rendimiento para {symbol}: {e} Por gregorbc@gmail.com"
                )
                db.rollback()
                return None
            finally:
        ...
        def optimize_operation():
            ...
            try:
                ...
                for symbol, loss_count in recent_losing_trades:
                    total_trades = (
                        db.query(Trade).filter(Trade.symbol == symbol).count()
                    )
                    if total_trades > strategy_optimizer.MIN_TRADES_FOR_ANALYSIS:
                        loss_ratio = loss_count / total_trades
                        if loss_ratio > 0.7:
                            log.warning(
                                f"⚠️ Símbolo problemático detectado: {symbol} con {loss_ratio:.2%} de trades perdedores Por gregorbc@gmail.com"
                            )
            except Exception as e:
                log.error(
                    f"Error optimizando estrategia basada en pérdidas: {e} Por gregorbc@gmail.com"
                )
            finally:
                db.close()
    ...
    def analyze_problematic_symbols(self):
        ...
        try:
            ...
            for stat in symbol_stats:
                win_rate = (
                    stat.winning_trades / stat.total_trades
                    if stat.total_trades > 0
                    else 0
                )
            ...
            for symbol_data in problematic_symbols:
                ...
                log.warning(
                    f"⚠️ Símbolo problemático: {symbol}, Win Rate: {symbol_data['win_rate']:.2%}, PnL Total: {symbol_data['total_pnl']:.2f}"
                )
                ...
                if symbol_data["win_rate"] < 0.3:
                    ...
                    log.info(
                        f"🔒 Deshabilitando trading para {symbol} debido a mal performance Por gregorbc@gmail.com"
                    )
                else:
                    # Reducir riesgo
                    config.SYMBOL_SPECIFIC_SETTINGS[symbol] = {
                    ...
                    log.info(
                        f"⚠️ Reduciendo riesgo para {symbol} debido a performance mediocre Por gregorbc@gmail.com"
                    )

        except Exception as e:
            log.error(
                f"Error analizando símbolos problemáticos: {e} Por gregorbc@gmail.com"
            )
        finally:
            if "db" in locals():
                db.close()
            ...
            try:
                ...
                daily_trades = (
                    db.query(Trade).filter(
                        Trade.timestamp >= start_of_day).all()
                )
...
"""
                ...
                for symbol, trades in by_symbol.items():
                    ...
                    symbol_win_rate = (
                        len([t for t in trades if t.pnl > 0]) /
                        len(trades) * 100
                    )
                ...
                log.info(
                    f"📊 Reporte diario generado: {total_trades} trades, P&L Diario: {total_pnl:+.2f} USDT (20x) Por gregorbc@gmail.com"
                )

            except Exception as e:
                log.error(
                    f"Error generando reporte diario: {e} Por gregorbc@gmail.com")
            finally:
                db.close()

    def open_trade(self, symbol: str, side: str, last_candle):
        # Verificar spread excesivo
        ticker = self.api._safe_api_call(
            self.api.client.futures_symbol_ticker, symbol=symbol
        )
        if ticker:
            ...
            if bid_price > 0 and ask_price > 0:
                ...
                if spread > config.MAX_SPREAD_PERCENT:
                    log.warning(
                        f"⏭️ Spread demasiado alto ({spread:.2f}%) en {symbol}, omitiendo trade Por gregorbc@gmail.com"
                    )
                    return
        ...
        if not symbol_settings.get("enabled", True):
            log.info(
                f"⏭️ Símbolo {symbol} deshabilitado en configuración Por gregorbc@gmail.com"
            )
            return
        ...
        if min_position_size * price > self.capital_manager.current_balance:
            log.info(
                f"⏭️ {symbol} requiere mínimo {min_position_size * price:.2f} USDT, capital insuficiente Por gregorbc@gmail.com"
            )
            return
        ...
        if config.DRY_RUN:
            log.info(
                f"[DRY RUN] Would open {side} on {symbol} Por gregorbc@gmail.com")
            return
        ...
        if quantity <= 0:
            log.info(
                f"⏭️ Tamaño de posición inválido para {symbol} Por gregorbc@gmail.com"
            )
            return
        ...
        if quantity < min_qty:
            log.info(
                f"⏭️ Cantidad {quantity} para {symbol} es menor que el mínimo {min_qty} Por gregorbc@gmail.com"
            )
            return
        ...
        if notional_value < min_notional:
            log.info(
                f"⏭️ Valor nocional {notional_value:.2f} para {symbol} es menor que el mínimo {min_notional:.2f} Por gregorbc@gmail.com"
            )
            return
        ...
        log.info(
            f"Attempting to place MARKET order for {side} {symbol} (Qty: {quantity}, Value: {notional_value:.2f} USDT, 20x) Por gregorbc@gmail.com"
        )
        ...
        if order and order.get("orderId"):
            log.info(
                f"✅ MARKET ORDER CREATED: {side} {quantity} {symbol} (20x) Por gregorbc@gmail.com"
            )
            ...
            log.error(
                f"❌ Could not create market order for {symbol}. Response: {order} Por gregorbc@gmail.com"
            )

    def run(self):
        log.info(
            f"🚀 STARTING TRADING BOT v12.0 WITH 20x LEVERAGE - USDT PAIRS Por gregorbc@gmail.com"
        )
        log.info(
            f"⚡ Configuración 20x: SL={config.STOP_LOSS_PERCENT}%, TP={config.TAKE_PROFIT_PERCENT}%"
        )
        log.info(
            f"⚡ Risk per trade: {config.RISK_PER_TRADE_PERCENT}%, Max positions: {config.MAX_CONCURRENT_POS}"
        )

        # Verificar conexión y configuración
        log.info(f"🔑 API Key presente: {bool(os.getenv('BINANCE_API_KEY'))}")
        log.info(
            f"🔑 API Secret presente: {bool(os.getenv('BINANCE_API_SECRET'))}")
        log.info(
            f"🌐 Modo Testnet: {os.getenv('BINANCE_TESTNET', 'true').lower() == 'true'}"
        )
        ...
        if DB_ENABLED:
            ...
            log.info(
                f"🗄️ Conexión a base de datos: {'✅ SALUDABLE' if db_healthy else '❌ PROBLEMAS'}"
            )
        ...
        if self.capital_manager.current_balance < config.MIN_BALANCE_THRESHOLD:
            log.warning(
                f"⚠️ Balance insuficiente: {self.capital_manager.current_balance:.2f} USDT < {config.MIN_BALANCE_THRESHOLD} USDT"
            )
            if self.telegram_notifier.enabled:
        ...
        while True:
            ...
            try:
                self.cycle_count += 1
                log.info(
                    f"--- 🔄 Cycle {self.cycle_count} - Capital: {self.capital_manager.current_balance:.2f} USDT (20x) ---"
                )
                ...
                if self.cycle_count % config.CAPITAL_UPDATE_INTERVAL == 0:
                    if self.capital_manager.update_balance():
                        ...
                        profit_percentage = (
                            (
                                self.capital_manager.total_profit
                                / self.capital_manager.initial_balance
                                * 100
                            )
                            if self.capital_manager.initial_balance > 0
                            else 0
                        )
                        ...
                        with state_lock:
                            ...
                            app_state["capital_stats"] = (
                                self.capital_manager.get_performance_stats()
                            )
                ...
                if current_date != self.today:
                    ...
                    self.daily_pnl = 0.0
                    with state_lock:
                        app_state["daily_starting_balance"] = (
                            self.capital_manager.current_balance
                        )
                        app_state["daily_pnl"] = 0.0
                    ...
                    log.info(
                        "🔄 Nuevo día - Reiniciando métricas diarias (20x) Por gregorbc@gmail.com"
                    )

                with state_lock:
                    app_state["connection_metrics"]["uptime"] = (
                ...
                if self.cycle_count % 12 == 0:
                    ...
                    log.info(
                        f"🏛️ Market Regime: {self.market_regime} (20x) Por gregorbc@gmail.com"
                    )
                ...
                with state_lock:
                    recent_trades = app_state["trades_history"][
                    ...
                    if consecutive_losses >= config.MAX_CONSECUTIVE_LOSSES:
                        log.warning(
                            f"⛔ {consecutive_losses} pérdidas consecutivas, pausando trading por 1 hora Por gregorbc@gmail.com"
                        )
                        time.sleep(3600)
                        continue

                if self.daily_pnl < -(
                    self.daily_starting_balance * config.MAX_DAILY_LOSS_PERCENT / 100
                ):
                    log.warning(
                        f"⛔ Límite de pérdida diaria alcanzado: {self.daily_pnl:.2f} USDT (20x) Por gregorbc@gmail.com"
                    )
                    if self.telegram_notifier.enabled:
                        self.telegram_notifier.notify_warning(
                ...
                if low_balance:
                    log.warning(
                        "⏸️ Pausando nuevas operaciones por balance bajo (20x) Por gregorbc@gmail.com"
                    )
                    if self.telegram_notifier.enabled:
                        self.telegram_notifier.notify_warning(
                ...
                for symbol, position in open_positions.items():
                    try:
                        ...
                        exit_signal = self.dynamic_exit_strategy(
                            symbol, position, current_price, df
                        )

                        if exit_signal:
                            close_order = self.api.close_position(
                                symbol, float(position["positionAmt"])
                            )
                            if close_order:
                                sl_tp_data = app_state["sl_tp_data"].get(
                                    symbol, {})
                                sl_adj = sl_tp_data.get("sl_adjustment", 1.0)
                                tp_adj = sl_tp_data.get("tp_adjustment", 1.0)

                                trade_result = _record_closed_trade(
                                    self.api,
                                    symbol,
                                    position,
                                    close_order.get("orderId"),
                                    exit_signal.lower(),
                                    current_price,
                                )
                                ...
                                if trade_result is not None:
                                    ...
                                    self.capital_manager.reinvest_profit(
                                        trade_result)
                                ...
                                if (
                                    exit_signal.lower() in [
                                        "sl", "tp", "trailing"]
                                    and trade_result is not None
                                ):
                        ...
                        log.error(
                            f"Error checking stops for {symbol}: {e} Por gregorbc@gmail.com",
                            exc_info=True,
                        )
                        if self.telegram_notifier.enabled:
                            self.telegram_notifier.notify_error(
                                f"Error verificando stops para {symbol}", str(
                                    e)
                            )
                ...
                if self.cycle_count % 6 == 0 and DB_ENABLED:
                    try:
                        ...
                        log.error(
                            f"Error en análisis de rendimiento: {e} Por gregorbc@gmail.com"
                        )
                ...
                if num_open_pos < config.MAX_CONCURRENT_POS and not low_balance:
                    ...
                    log.info(
                        f"🔍 Scanning {len(symbols_to_scan)} symbols for new signals (20x). Por gregorbc@gmail.com"
                    )
                    ...
                    with ThreadPoolExecutor(
                        ...
                        for future in futures:
                            ...
                            if signal:
                                log.info(
                                    f"🔥 Signal found! {signal} on {symbol} (20x) Por gregorbc@gmail.com"
                                )
                                ...
                                if len(open_positions) + 1 >= config.MAX_CONCURRENT_POS:
                                    log.info(
                                        "🚫 Concurrent positions limit reached (20x). Por gregorbc@gmail.com"
                                    )
                                    break
                ...
                with state_lock:
                    ...
                    if trades:
                        ...
                        winning_trades_pnl = [t["pnl"]
                                              for t in trades if t["pnl"] > 0]
                        losing_trades_pnl = [t["pnl"]
                                             for t in trades if t["pnl"] < 0]
                        stats["avg_win"] = (
                            sum(winning_trades_pnl) / len(winning_trades_pnl)
                            if winning_trades_pnl
                            else 0
                        )
                        stats["avg_loss"] = (
                            abs(sum(losing_trades_pnl) / len(losing_trades_pnl))
                        ...
                        stats["profit_factor"] = (
                            total_win / total_loss if total_loss > 0 else total_win
                        )
                        ...
                        if durations:
                            stats["avg_trade_duration"] = sum(durations) / len(
                                durations
                            )
                ...
                log.error(
                    f"Error in main loop: {e} Por gregorbc@gmail.com", exc_info=True
                )
                if self.telegram_notifier.enabled:
                    self.telegram_notifier.notify_error(
                        "Error en el loop principal del bot", str(e)
                    )
...
def _record_closed_trade(
    ...
    try:
        pnl_records = api._safe_api_call(
            api.client.futures_account_trades, symbol=symbol, limit=20
        )
        realized_pnl = 0.0
        ...
        roe = (
            (realized_pnl / (position_size * entry_price / current_leverage)) * 100
            if entry_price > 0 and position_size > 0 and current_leverage > 0
            else 0.0
        )

        entry_timestamp = int(position_data.get("updateTime", 0))
        entry_time = (
            datetime.fromtimestamp(entry_timestamp / 1000)
        ...
        if DB_ENABLED:
            ...
            def save_trade_operation():
                ...
                try:
                    ...
                    db.commit()
                    log.info(
                        f"💾 Trade guardado en DB: {symbol}, PnL: {realized_pnl:.2f}"
                    )
                except Exception as e:
                    log.error(
                        f"DB Error recording trade for {symbol}: {e} Por gregorbc@gmail.com"
                    )
                    db.rollback()
                finally:
                    db.close()

            execute_with_retry(save_trade_operation,
                               max_retries=2, retry_delay=1)

        log.info(
            f"✅ Position closed: {symbol}, PnL: {realized_pnl:.2f} USDT, ROE: {roe:.2f}%, Reason: {close_type} (20x) Por gregorbc@gmail.com"
        )
        socketio.emit("status_update", app_state)
        ...
        log.error(
            f"❌ Critical error in _record_closed_trade for {symbol}: {e}",
            exc_info=True,
        )
        return None
...
def start_bot():
    ...
    with state_lock:
        if app_state["running"]:
            return (
                jsonify({"status": "error", "message": "Bot is already running"}),
                400,
            )
...
def update_config():
        ...
        for key, value in data.items():
            if hasattr(config, key):
                try:
                    ...
                    log.warning(
                        f"Failed to set config field {key} with value {value}: {e} Por gregorbc@gmail.com"
                    )
                    return (
                        jsonify(
                            {
    ...
    log.info(
        f"⚙️ Configuration updated: {updated_fields} (20x) Por gregorbc@gmail.com")
...
def close_position_api():
    ...
    try:
        ...
        if close_order:
            ticker = api._safe_api_call(
                api.client.futures_symbol_ticker, symbol=symbol)
            exit_price = (
                float(ticker["price"]) if ticker else float(
                    position["entryPrice"])
            )
            pnl = _record_closed_trade(
                api, symbol, position, close_order.get(
                    "orderId"), "manual", exit_price
            )
            ...
            return jsonify(
                {"status": "success", "message": f"Position on {symbol} closed."}
            )
        else:
            return (
                jsonify(
                    {"status": "error", "message": "Failed to send close order."}),
                500,
            )

    except Exception as e:
        log.error(
            f"Error closing position {symbol}: {e} Por gregorbc@gmail.com")
...
def manual_trade():
    ...
    try:
        ...
        if not mark:
            return (
                jsonify(
                    {"status": "error", "message": "Unable to fetch mark price."}),
                500,
            )
        ...
        if not filters:
            return (
                jsonify(
                    {
                        "status": "error",
                        "message": f"Could not get filters for {symbol}",
                    }
                ),
                500,
            )
        leverage_param = float(
            data.get("leverage", config.LEVERAGE) or config.LEVERAGE)
        quantity = api.round_value(
            (margin * leverage_param) / price, filters["stepSize"]
        )
        if quantity < filters["minQty"] or (quantity * price) < filters["minNotional"]:
            return (
                jsonify(
                    {
                        "status": "error",
                        "message": f"Quantity ({quantity}) below minimum allowed.",
                    }
                ),
                400,
            )
        ...
        if order and (order.get("orderId") or order.get("mock")):
            log.info(
                f"MANUAL TRADE CREATED: {side} {quantity} {symbol} (20x) Por gregorbc@gmail.com"
            )
            return jsonify(
                {
                    "status": "success",
                    "message": f"Manual market order for {symbol} created.",
                }
            )
        else:
            return (
                jsonify(
                    {"status": "error", "message": f"Manual order failed: {order}"}
                ),
                500,
            )
    except Exception as e:
...
def get_trade_history():
    with state_lock:
        trades = app_state["trades_history"]
    sorted_trades = sorted(trades, key=lambda x: x.get(
...
def get_performance_metrics():
    ...
    try:
        ...
        log.error(
            f"Error fetching performance metrics: {e} Por gregorbc@gmail.com")
    ...
    try:
        ...
        symbol_stats = (
            db.query(
                ...
                func.avg(case((Trade.pnl > 0, Trade.pnl),
                         else_=None)).label("avg_win"),
                func.avg(case((Trade.pnl < 0, Trade.pnl), else_=None)).label(
                    "avg_loss"
                ),
                func.sum(Trade.pnl).label("total_pnl"),
            )
        ...
        log.error(
            f"Error fetching performance stats: {e} Por gregorbc@gmail.com")
        return jsonify({"error": str(e)}), 500
    finally:
        if "db" in locals():
            db.close()
...
def get_detailed_metrics():
    ...
    try:
        with state_lock:
            ...
            metrics = {
                ...
                "profit_factor": (
                    ...
                    if losing_trades
                    and sum(t.get("pnl", 0) for t in losing_trades) != 0
                    else float("inf")
                ),
                "avg_trade_duration": app_state["performance_stats"].get(
        ...
        log.error(
            f"Error calculando métricas detalladas: {e} Por gregorbc@gmail.com")
        return jsonify({"error": str(e)}), 500
    ...
    try:
        ...
        if bot_instance.capital_manager.update_balance(force=True):
            with state_lock:
                ...
                app_state["capital_stats"] = (
                    bot_instance.capital_manager.get_performance_stats()
                )
            return jsonify(
            ...
            return (
                jsonify({"status": "error", "message": "Failed to update balance"}),
                500,
            )
    except Exception as e:
...
def get_db_trade_history():
    ...
    try:
        ...
        log.error(
            f"Error fetching trade history from DB: {e} Por gregorbc@gmail.com")
        return jsonify({"error": str(e)}), 500
    finally:
        if "db" in locals():
            db.close()
