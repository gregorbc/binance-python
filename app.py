    ...
    def _safe_api_call(self, func, *args, **kwargs):
        for attempt in range(3):
            try:
                ...
                if e.code == -4131:
                    log.warning(
                        f"PERCENT_PRICE error (-4131) in order. Volatile or illiquid market. Skipping."
                    )
                    return None
            ...
            try:
                ...
                with state_lock:
                    ...
                    if stats["trades_count"] > 0:
                        ...
                        stats["avg_loss"] = (
                            total_loss / stats["losses"] if stats["losses"] > 0 else 0
                        )
...
def close_position_api():
    ...
    try:
        ...
        if result:
            ...
            log.info(
                f"✅ Position on {symbol} closed. Realized PNL: {realized_pnl:.2f} USDT"
            )
            return jsonify(
...
def manual_trade():
    ...
    try:
        ...
        if order and order.get("orderId"):
            log.info(
                f"MANUAL TRADE CREATED: {side} {quantity} {symbol} @ {limit_price}"
            )
            return jsonify(
                {
                    "status": "success",
                    "message": f"Manual limit order for {symbol} created.",
                }
            )
        else:
            return (
