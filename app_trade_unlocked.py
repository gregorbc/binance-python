...
class TradingBot:
        ...
        try:
            ...
            for t in tickers:
                ...
                if (symbol.endswith('USDT') and
                    ...
                    float(t.get('quoteVolume', 0)) > config.MIN_24H_VOLUME):
                    valid_symbols.append(symbol)

            # Ordenar por volumen y limitar
            valid_symbols.sort(key=lambda x: float(next(t['quoteVolume'] for t in tickers if t['symbol'] == x)), reverse=True)
    ...
    def calculate_position_size(self, symbol: str, price: float) -> float:
        ...
        filters = self.api.get_symbol_filters(symbol) if hasattr(self.api, "get_symbol_filters") else None
        min_notional = 5.0
        min_qty = None
        if filters:
            try:
        ...
        if risk_amount < min_risk_needed:
            log.debug(f"Ajustando riesgo de {risk_amount:.4f} -> {min_risk_needed:.4f} (minNotional={min_notional}, L={leverage})")
    ...
    def get_open_positions(self):
        ...
        try:
            account_info = self.api._safe_api_call(self.api.client.futures_account)
            if account_info:
                return {p['symbol']: p for p in account_info['positions'] if float(p['positionAmt']) != 0}
            return {}
        except Exception as e:
