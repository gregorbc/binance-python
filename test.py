...
class CONFIG:
    ...
    FIXED_MARGIN_PER_TRADE_USDT: float = 2.0 # Aumentado para evitar errores de cantidad mínima
...
if not log.handlers:
    ...
    log.addHandler(file_handler); log.addHandler(socket_handler); log.addHandler(console_handler)
...
bot_thread = None
app_state = {
    "running": False, "status_message": "Detenido", "open_positions": {},
    "config": asdict(config),
    "performance_stats": {"realized_pnl": 0.0, "trades_count": 0},
    "balance": 0.0, "total_investment_usd": 0.0
}
...
class BinanceFutures:
    def __init__(self):
        ...
        if not api_key or not api_secret: raise ValueError("API keys no configuradas en .env")
        ...
        try:
            ...
            log.error(f"❌ Error conectando a Binance: {e}"); raise
    ...
    def _safe_api_call(self, func, *args, **kwargs):
        for attempt in range(3):
            try:
                ...
                if e.code == -4131:
                    log.warning(f"Error PERCENT_PRICE (-4131) en orden. Mercado volátil o ilíquido. Omitiendo.")
                    return None
                log.warning(f"API non-critical error: {e.message}")
                if attempt == 2: log.error(f"Error final de API: {e.code} - {e.message}")
                time.sleep(1 * (attempt + 1))
            except Exception as e:
                log.warning(f"Error general en llamada API: {e}")
                if attempt == 2: log.error(f"Error general final: {e}")
                time.sleep(1 * (attempt + 1))
        return None
    ...
    def place_order(self, symbol: str, side: str, order_type: str, quantity: float, price: Optional[float] = None, reduce_only: bool = False) -> Optional[Dict]:
        ...
        if order_type == FUTURE_ORDER_TYPE_LIMIT:
            if price is None: log.error("Precio requerido para órdenes LIMIT."); return None
...
class TradingBot:
    ...
    def get_top_symbols(self) -> List[str]:
        ...
        valid = [t for t in tickers if t['symbol'].endswith('USDT') and t['symbol'] not in config.EXCLUDE_SYMBOLS and float(t['quoteVolume']) > config.MIN_24H_VOLUME]
        return [t['symbol'] for t in sorted(valid, key=lambda x: float(x['quoteVolume']), reverse=True)[:config.NUM_SYMBOLS_TO_SCAN]]
    ...
    def check_signal(self, df: pd.DataFrame) -> Optional[str]:
        ...
        if last['fast_ema'] > last['slow_ema'] and prev['fast_ema'] <= prev['slow_ema'] and last['rsi'] > 50: return 'LONG'
        if last['fast_ema'] < last['slow_ema'] and prev['fast_ema'] >= prev['slow_ema'] and last['rsi'] < 50: return 'SHORT'
        return None
        ...
        while True:
            ...
            try:
                ...
                if open_positions:
                    ...
                    socketio.emit('pnl_update', {p['symbol']: float(p['unrealizedProfit']) for p in open_positions.values()})
                ...
                if num_open_pos < config.MAX_CONCURRENT_POS:
                    ...
                    log.info(f"🔍 Escaneando {len(symbols_to_scan)} símbolos para nuevas señales.")
                        ...
                        for future in futures:
                            ...
                            if (signal := self.check_signal(df)):
                                ...
                                if len(open_positions) + 1 >= config.MAX_CONCURRENT_POS:
                                    log.info(" Límite de posiciones concurrentes alcanzado.")
                                    break
            ...
            except Exception as e: log.error(f"Error en ciclo principal: {e}", exc_info=True)
            time.sleep(config.POLL_SEC)
        log.info("🛑 Bot detenido.")

    def open_trade(self, symbol: str, side: str, last_candle):
        if config.DRY_RUN: log.info(f"[DRY RUN] Abriría {side} en {symbol}"); return
        ...
        if quantity < filters['minQty'] or (quantity * price) < filters['minNotional']:
            log.warning(f"Cantidad {quantity} para {symbol} es menor al mínimo permitido."); return
        ...
        if order and order.get('orderId'):
            log.info(f"✅ ORDEN LÍMITE CREADA: {side} {quantity} {symbol} @ {limit_price}")
        else:
            log.error(f"❌ No se pudo crear la orden límite para {symbol}. Respuesta: {order}")
...
def start_bot():
    global bot_thread
    with state_lock:
        if app_state["running"]: return jsonify({"status": "error", "message": "El bot ya está en ejecución"}), 400
        app_state["running"] = True; app_state["status_message"] = "Iniciando..."
...
def stop_bot():
    with state_lock:
        if not app_state["running"]: return jsonify({"status": "error", "message": "El bot no está en ejecución"}), 400
        app_state["running"] = False; app_state["status_message"] = "Deteniendo..."
...
def update_config():
    ...
    return jsonify({"status": "success", "message": "Configuración guardada.", "config": app_state["config"]})

@app.route('/api/close_position', methods=['POST'])
def close_position_api():
    symbol = request.json.get('symbol')
    if not symbol: return jsonify({"status": "error", "message": "Falta el símbolo"}), 400
    
    with state_lock:
        position = app_state["open_positions"].get(symbol)
    
    if not position: return jsonify({"status": "error", "message": f"No se encontró posición para {symbol}"}), 404
    ...
    try:
        ...
        if result:
            ...
            log.info(f"✅ Posición en {symbol} cerrada. PNL Realizado: {realized_pnl:.2f} USDT")
            return jsonify({"status": "success", "message": f"Posición en {symbol} cerrada."})
        else:
            return jsonify({"status": "error", "message": "Falló el envío de la orden de cierre."}), 500
    except Exception as e:
...
def manual_trade():
    ...
    if not all([symbol, side]): return jsonify({"status": "error", "message": "Faltan parámetros."}), 400
    ...
    try:
        ...
        if not filters: return jsonify({"status": "error", "message": f"No se obtuvieron filtros para {symbol}"}), 500
        
        quantity = (margin * config.LEVERAGE) / price
        quantity = api.round_value(quantity, filters['stepSize'])
        if quantity < filters['minQty'] or (quantity * price) < filters['minNotional']:
            return jsonify({"status": "error", "message": f"Cantidad ({quantity}) menor al mínimo permitido."}), 400
        
        order_side = SIDE_BUY if side == 'LONG' else SIDE_SELL
        tick_size = filters['tickSize']
        limit_price = api.round_value(price + tick_size * 5 if side == 'LONG' else price - tick_size * 5, tick_size)
        ...
        if order and order.get('orderId'):
            ...
            return jsonify({"status": "success", "message": f"Orden límite manual para {symbol} creada."})
        else:
            return jsonify({"status": "error", "message": f"La orden manual falló: {order}"}), 500
    except Exception as e:
