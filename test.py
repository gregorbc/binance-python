...
class BinanceFutures:
    def __init__(self):
        ...
        if not api_key or not api_secret:
            raise ValueError(
                "API keys no configuradas. Crea un archivo .env con BINANCE_API_KEY y BINANCE_API_SECRET"
            )
    ...
    def _is_cache_valid(self, symbol: str) -> bool:
        return symbol in self.cache_expiry and time.time() < self.cache_expiry[symbol]
        ...
        for attempt in range(max_retries):
            try:
                ...
                if e.code in non_critical_codes:
                    ...
                    log.debug(
                        f"API {func.__name__} para {symbol_name}: {non_critical_codes.get(e.code, e.message)}"
                    )
                    ...
                    if attempt == max_retries - 1:
                        log.error(
                            f"Error API Binance en {func.__name__}: {e.code} - {e.message}"
                        )
                        return None
    ...
    def get_symbol_configuration(self, symbol: str) -> Dict:
        try:
            ...
            position_info = self._safe_api_call(
    ...
    def set_leverage_and_margin(self, symbol: str) -> bool:
        try:
            if symbol in self.symbol_config_cache and self._is_cache_valid(symbol):
                return True
            ...
            if needs_leverage_change:
                ...
                if result is not None:
                    log.info(
                        f"⚡ Leverage configurado para {symbol}: {config.LEVERAGE}x"
                    )
            ...
            if needs_margin_change:
                ...
                if result is not None:
                    log.info(
                        f"📊 Tipo de margen configurado para {symbol}: {config.MARGIN_TYPE}"
                    )
                    changes_made.append("margin")
                    if symbol in self.symbol_info_cache:
                        self.symbol_info_cache[symbol]["marginType"] = (
                            config.MARGIN_TYPE
                        )
                else:
                    if symbol in self.symbol_info_cache:
        ...
        try:
            ...
            for ticker in tickers:
                ...
                if (
                    ...
                    and float(ticker["quoteVolume"]) > config.MIN_24H_VOLUME
                ):
                    usdt_pairs.append(
                        {"symbol": symbol, "volume": float(ticker["quoteVolume"])}
                    )
...
class TradingStrategy:
    ...
    def add_technical_indicators(df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            df["volume_ma"] = (
                df["volume"].rolling(window=config.VOLUME_AVG_PERIOD).mean()
            )
...
class PositionManager:
    ...
    def calculate_position_size(
        api: BinanceFutures, symbol: str, price: float
    ) -> float:
        try:
            if price <= 0:
                return 0.0
            ...
            if quantity < min_qty:
                log.warning(
                    f"Cantidad {quantity} menor que mínimo {min_qty} para {symbol}"
                )
                return 0.0
            ...
            if notional < min_notional:
                log.warning(
                    f"Valor nocional {notional:.2f} menor que mínimo {min_notional} para {symbol}"
                )
                return 0.0
...
class TradingBot:
    ...
    def run(self):
        ...
        while app_state["running"]:
            ...
            try:
                ...
                log.info(
                    f"📊 Ciclo {self.cycle_count} - Balance: {app_state['balance']:.2f} USDT"
                )
                ...
                if len(open_positions) >= config.MAX_CONCURRENT_POS:
                    log.info(
                        f"🔒 Límite de posiciones alcanzado ({len(open_positions)}/{config.MAX_CONCURRENT_POS})"
                    )
                    with state_lock:
                ...
                symbols_to_scan = self.api.get_top_symbols_by_volume(
                ...
                with state_lock:
                    app_state["status_message"] = (
                        f"Esperando {sleep_time:.0f}s para próximo ciclo..."
                    )

                log.info(
                    f"⏱️ Ciclo completado en {cycle_duration:.2f}s. Esperando {sleep_time:.0f}s..."
                )
    ...
    def _configure_symbols_intelligently(
        self, symbols: List[str], open_positions: Dict
    ):
        try:
            ...
            for symbol in symbols[:20]:
                if (
                    ...
                    and symbol not in self.api.symbol_config_cache
                ):
                    symbols_to_configure.append(symbol)

            if symbols_to_configure:
                log.info(
                    f"🔧 Configurando {len(symbols_to_configure)} símbolos nuevos..."
                )
                configured_count = 0
                ...
                if configured_count > 0:
                    log.info(
                        f"✅ {configured_count} símbolos configurados correctamente"
                    )
            else:
    ...
    def _update_performance_stats(self, open_positions: Dict):
        try:
            ...
            if start_balance > 0:
                ...
                if total_trades > 0:
                    win_rate = (
                        self.performance_tracker["winning_trades"] / total_trades
                    ) * 100
    ...
    def _scan_for_signals(self, symbols: List[str], open_positions: Dict) -> List[Dict]:
        try:
            ...
            klines_data = self.api.get_klines_batch(
                symbols_to_check, config.TIMEFRAME, config.CANDLES_LIMIT
            )
            ...
            for symbol, df in klines_data.items():
                try:
                    ...
                    signal_type = TradingStrategy.detect_signal(
                    ...
                    if signal_type:
                        ...
                        if (
                            last_signal_key in self.last_signal_time
                            and current_time - self.last_signal_time[last_signal_key]
                            < 300
                        ):
                            continue
                        ...
                        strength = TradingStrategy.calculate_signal_strength(
                            df_with_indicators, signal_type
                        )
                        ...
                        log.info(
                            f"🎯 Señal {signal_type} detectada en {symbol} (fuerza: {strength:.2f})"
                        )
    ...
    def _execute_best_signals(self, signals: List[Dict], open_positions: Dict):
        try:
            ...
            for signal in signals:
                ...
                quantity = PositionManager.calculate_position_size(
                    self.api, symbol, price
                )
                ...
                log.info(
                    f"🚀 Ejecutando {signal_type} en {symbol}: {quantity} @ {price:.4f}"
                )
                ...
                if order and order.get("status") == "FILLED":
                    ...
                    log.info(
                        f"✅ Posición abierta en {symbol}: {signal_type} {quantity} @ {price:.4f}"
                    )
            ...
            for symbol in symbols[:5]:
                try:
                    ticker = self.api._safe_api_call(
                        self.api.client.futures_ticker, symbol=symbol
                    )
                    if ticker:
                        top_coins.append(
                            {
...
def start_bot():
    ...
    with state_lock:
        if app_state["running"]:
            return jsonify(
                {"status": "error", "message": "El bot ya está ejecutándose"}
            )
...
def stop_bot():
    with state_lock:
        if not app_state["running"]:
            return jsonify(
                {"status": "error", "message": "El bot no está ejecutándose"}
            )
...
def manual_trade():
    try:
        ...
        if not ticker:
            return jsonify(
                {"status": "error", "message": f"Símbolo {symbol} no válido"}
            )
        ...
        if order and order.get("status") == "FILLED":
            ...
            log.info(
                f"📱 Trade manual ejecutado: {side} {quantity} {symbol} @ {price:.4f}"
            )
            return jsonify(
                {
                    "status": "success",
                    "message": f"Trade ejecutado: {side} {quantity} {symbol}",
                }
            )
        else:
...
def close_position():
    try:
        ...
        if symbol not in positions:
            return jsonify(
                {"status": "error", "message": f"No hay posición abierta para {symbol}"}
            )
        ...
        if position_amt == 0:
            return jsonify(
                {"status": "error", "message": f"Posición ya cerrada para {symbol}"}
            )
        ...
        if order and order.get("status") == "FILLED":
            try:
                api._safe_api_call(
                    api.client.futures_cancel_all_open_orders, symbol=symbol
                )
            except:
                pass
            ...
            return jsonify(
                {"status": "success", "message": f"Posición cerrada: {symbol}"}
            )
        else:
...
def get_balance():
    try:
        ...
        return jsonify(
            {"status": "success", "balance": balance, "account_info": account_info}
        )
...
def main():
    ...
    try:
        log.info(
            "🚀 Iniciando servidor web del Bot de Binance Futures v7.3 (Configuración Inteligente)"
        )
