...
def setup_logging():
    ...
    file_handler = logging.FileHandler(
        f'logs/{getattr(config, "LOG_FILE", "bot.log")}', 
        encoding='utf-8'
    )
...
class CONFIG:
    ...
    EXCLUDE_SYMBOLS: tuple = field(default_factory=lambda: (
        "BTCDOMUSDT", "DEFIUSDT", "USDCUSDT", "1000BONKUSDT", "1000FLOKIUSDT",
        "1000LUNCUSDT", "1000PEPEUSDT", "1000SHIBUSDT", "1000XECUSDT"
    ))
...
class ErrorRecoverySystem:
    ...
    def handle_error(self, error: Exception, context: str = ""):
        ...
        if self.error_count >= self.max_errors_per_hour:
            log.warning("🛑 Muchos errores recientes, pausando operaciones por 10 minutos")
...
class TradingBot:
    ...
    def _check_market_conditions(self) -> bool:
        ...
        daily_pnl = self.capital_manager.current_balance - self.capital_manager.daily_starting_balance
        daily_loss_limit = self.capital_manager.daily_starting_balance * config.MAX_DAILY_LOSS_PERCENT / 100
    ...
    def _cleanup_memory(self):
        ...
        self.recently_signaled = {
            sym for sym, timestamp in self.recently_signaled.items()
    ...
    def _calculate_spread_risk(self, symbol: str) -> float:
        ...
        try:
            ticker = self.bot.api._safe_api_call(
                self.bot.api.client.futures_symbol_ticker, 
                symbol=symbol
            )
            if ticker:
...
def api_risk_metrics():
    ...
    with state_lock:
        return jsonify({
            ...
            "exposure_ratio": app_state.get("risk_metrics", {}).get("exposure_ratio", 0),
            "max_drawdown": app_state.get("risk_metrics", {}).get("max_drawdown", 0),
