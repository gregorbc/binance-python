...
class BinanceFuturesMaxProfit:
    def __init__(self):
        ...
        log.info(
            f"🔧 Conectado a Binance Futures {'TESTNET' if testnet else 'MAINNET'} - 9.81 USDT MAXIMO RENDIMIENTO"
        )
    ...
    def get_account_info(self):
        try:
            ...
            if account:
                ...
                log.info(
                    f"💰 Balance 9.81 USDT MAXPROFIT: Total: {total_balance:.2f} USDT, Disponible: {available_balance:.2f} USDT"
                )
    ...
    def _extract_precision_from_symbol_info(self, symbol_info: dict) -> dict:
        ...
        if lot_size:
            ...
            quantity_precision = (
                0 if step_size >= 1 else int(round(-math.log10(step_size)))
            )
        else:
            step_size = 0.001
            min_qty = 0.001
        ...
        if price_f:
            ...
            price_precision = (
                0 if tick_size >= 1 else int(round(-math.log10(tick_size)))
            )
        else:
            tick_size = 0.01
            price_precision = 2
    ...
    def place_order_with_retry(
        self, symbol: str, side: str, quantity: float, max_retries: int = 3
    ):
            ...
            try:
                ...
                if symbol in self.failed_symbols and attempt > 0:
                    ...
                    if adjusted_notional >= 6.0:
                        ...
                        log.info(
                            f"🔧 Cantidad reducida por errores previos: {quantity:.0f}"
                        )
                ...
                if "MAX_POSITION" in str(e) or "LEVERAGE" in str(e) or e.code == -2027:
                    ...
                    log.warning(
                        f"🔧 Límite de posición/exceso de leverage detectado en {symbol}"
                    )

                    if attempt == max_retries - 1:
                        log.error(
                            f"❌ Error persistente de límite en {symbol}, añadiendo a lista negra temporal"
                        )
                        return None
                    ...
                    if attempt == max_retries - 1:
                        log.error(
                            f"❌ Error persistente de margen en {symbol}, añadiendo a lista negra temporal"
                        )
                        return None
                    ...
                    if new_notional >= min_notional:
                        ...
                        if new_notional >= min_notional:
                            quantity = adjusted_qty
                            log.info(
                                f"🔧 Cantidad ajustada: {quantity} = ${new_notional:.2f}"
                            )
                        else:
                            log.error(
                                f"❌ No se puede alcanzar notional mínimo para {symbol}"
                            )
                            return None
    ...
    def place_order(self, symbol: str, side: str, quantity: float):
        ...
        try:
            ...
            if quantity < min_qty:
                log.warning(
                    f"⚠️ Cantidad {quantity} menor que mínimo {min_qty} para {symbol}"
                )
                return None
            ...
            if config.DRY_RUN:
                ...
                log.info(
                    f"[DRY_RUN] 🎯 Orden simulada: {side} {quantity} {symbol} @ ${current_price:.2f} = ${notional:.2f}"
                )
            ...
            if notional < MIN_NOTIONAL:
                log.warning(
                    f"⚠️ Notional insuficiente: ${notional:.2f} < ${MIN_NOTIONAL}"
                )
                min_quantity = MIN_NOTIONAL / current_price
                ...
                if min_notional >= MIN_NOTIONAL:
                    ...
                    log.info(
                        f"🔧 Usando cantidad mínima: {rounded_quantity} = ${notional:.2f}"
                    )
                else:
                    step_size = precision_info.get("step_size", 0.001)
                    ...
                    while notional < MIN_NOTIONAL and adjusted_quantity < 100000:
                        ...
                        adjusted_quantity = self.round_quantity(
                    ...
                    if notional >= MIN_NOTIONAL:
                        ...
                        log.info(
                            f"🔧 Ajustado a mínimo: {rounded_quantity} = ${notional:.2f}"
                        )
                    else:
                        log.warning(
                            f"⛔ Notional insuficiente incluso con ajustes: ${notional:.2f}"
                        )
                        return None

            log.info(
                f"📦 Colocando orden REAL para 9.81 USDT MAXPROFIT: {side} {rounded_quantity} {symbol} (Notional: ${notional:.2f})"
            )
    ...
    def place_stop_order(
        ...
        if config.DRY_RUN:
            log.info(
                f"[DRY_RUN] 🛡️ Orden stop simulada: {side} {quantity} {symbol} @ ${stop_price:.4f}"
            )
            return {"mock": True, "orderId": int(time.time() * 1000)}
        ...
        try:
            ...
            if price_distance_pct < config.MIN_STOP_DISTANCE_PCT:
                log.warning(
                    f"⭕ Stop demasiado cercano: {price_distance_pct:.3%} < {config.MIN_STOP_DISTANCE_PCT:.3%}"
                )
                if side == SIDE_SELL:
                    rounded_stop_price = current_price * (
                        1 - config.MIN_STOP_DISTANCE_PCT
                    )
                else:
                    rounded_stop_price = current_price * (
                        1 + config.MIN_STOP_DISTANCE_PCT
            ...
            log.info(
                f"🛡️ Colocando orden STOP para 9.81 USDT MAXPROFIT: {side} {rounded_quantity} {symbol} @ ${rounded_stop_price:.4f}"
            )
...
class MaxProfitCapitalManager:
        ...
        try:
            ...
            if account:
                ...
                if new_balance != self.current_balance:
                    ...
                    log.info(
                        f"💰 Balance 9.81 USDT MAXPROFIT actualizado: {self.current_balance:.2f} USDT"
                    )
    ...
    def can_trade_symbol_max_profit(self, symbol: str, current_price: float) -> bool:
        ...
        try:
            ...
            if test_notional < min_notional:
                ...
                while (
                    test_notional < min_notional
                    and adjusted_quantity < 100000
                    and max_iterations > 0
                ):
                    adjusted_quantity += step_size
                    adjusted_quantity = self.api.round_quantity(
                        symbol, adjusted_quantity
                    )
    ...
    def validate_position_limits(
        ...
        try:
            ...
            if notional < config.MIN_NOTIONAL_SAFETY:
                log.warning(
                    f"⭕ Notional de seguridad insuficiente: ${notional:.2f} < ${config.MIN_NOTIONAL_SAFETY:.2f}"
                )

                # 🔧 INTENTAR AJUSTAR LA CANTIDAD PARA CUMPLIR EL MÍNIMO
                min_quantity_safety = config.MIN_NOTIONAL_SAFETY / current_price
                min_quantity_safety = self.api.round_quantity(
                    symbol, min_quantity_safety
                )
                adjusted_notional = min_quantity_safety * current_price

                if adjusted_notional >= config.MIN_NOTIONAL_SAFETY:
                    log.info(
                        f"🔧 Cantidad ajustable para cumplir mínimo: {min_quantity_safety:.0f} = ${adjusted_notional:.2f}"
                    )
                    return True
                else:
                    return False
            ...
            if symbol in config.PROBLEMATIC_SYMBOLS:
                ...
                if notional > max_problematic_notional:
                    log.warning(
                        f"⭕ Símbolo problemático {symbol}, notional muy alto: ${notional:.2f}"
                    )
                    return False
    ...
    def calculate_max_profit_position_size(
        self,
        symbol: str,
        entry_price: float,
        stop_loss: float,
        ...
        try:
            ...
            log.info(
                f"💰 Margen disponible: ${available_balance:.2f} / Total: ${total_balance:.2f}"
            )
            ...
            if available_balance < MIN_AVAILABLE_MARGIN:
                log.warning(
                    f"⭕ Margen insuficiente: ${available_balance:.2f} < ${MIN_AVAILABLE_MARGIN:.2f}"
                )
                return 0.0
            ...
            min_notional = 6.0
            target_notional = min(
                7.5, available_balance * config.MAX_MARGIN_UTILIZATION
            )
            ...
            max_iterations = 500
            while (
                calculated_notional < min_notional
                and adjusted_quantity < 50000
                and max_iterations > 0
            ):
                adjusted_quantity += step_size
            ...
            if calculated_notional > available_balance * config.MAX_MARGIN_UTILIZATION:
                quantity = (
                    available_balance * config.MAX_MARGIN_UTILIZATION
                ) / entry_price
                quantity = self.api.round_quantity(symbol, quantity)
                calculated_notional = quantity * entry_price
                log.info(
                    f"🔧 Ajustando por límite de margen: {quantity:.0f} {symbol} = ${calculated_notional:.2f}"
                )
            ...
            if calculated_notional < config.MIN_NOTIONAL_SAFETY:
                ...
                if min_notional_safety >= config.MIN_NOTIONAL_SAFETY:
                    ...
                    log.info(
                        f"🔧 Ajustando para notional mínimo de seguridad: {quantity:.0f} {symbol} = ${calculated_notional:.2f}"
                    )
                else:
                    # Incrementar hasta alcanzar el mínimo
                    while (
                        calculated_notional < config.MIN_NOTIONAL_SAFETY
                        and quantity < 50000
                        and max_iterations > 0
                    ):
                        quantity += step_size
                    ...
                    if calculated_notional >= config.MIN_NOTIONAL_SAFETY:
                        log.info(
                            f"🔧 Incrementado para notional mínimo: {quantity:.0f} {symbol} = ${calculated_notional:.2f}"
                        )
                    else:
                        log.warning(
                            f"⭕ No se puede alcanzar notional mínimo de seguridad: ${calculated_notional:.2f} < ${config.MIN_NOTIONAL_SAFETY:.2f}"
                        )
                        return 0.0
            ...
            if symbol in self.api.failed_symbols:
                ...
                if calculated_notional < config.MIN_NOTIONAL_SAFETY:
                    log.warning(
                        f"⭕ Posición reducida por debajo del mínimo: ${calculated_notional:.2f} < ${config.MIN_NOTIONAL_SAFETY:.2f}"
                    )
                    return 0.0
            ...
            log.info(
                f"📊 Posición MAXIMO RENDIMIENTO: {quantity:.0f} {symbol} = ${calculated_notional:.2f} "
...
class MaxProfitIATradingSystem:
    ...
    def _get_minimum_confidence_prediction(self):
        ...
        return {"direction": "NEUTRAL", "confidence": 0.12, "aggressiveness": 0.08}
    ...
    def _calculate_max_profit_technical_signal(self, df: pd.DataFrame) -> dict:
        ...
        try:
            ...
            if ema_fast > ema_slow and rsi > 25 and rsi < 80 and macd_trend:
                direction = "BULLISH"
                strength = 0.7

            elif ema_fast < ema_slow and rsi < 75 and rsi > 20 and not macd_trend:
                direction = "BEARISH"
                strength = 0.7
    ...
    def _cleanup_old_cache(self):
        ...
        if current_time - self.last_cleanup > 60:
            expired_keys = [
                key
                for key in self.signal_cache.keys()
...
class UniversalSymbolScanner:
        ...
        for symbol in test_symbols:
            try:
                ...
                if current_price > 0 and current_price <= config.MAX_PRICE_USDT:
                    if self.capital_manager.can_trade_symbol_max_profit(
                        symbol, current_price
                    ):
                        guaranteed_symbols.append(symbol)
                        log.info(
                            f"✅ Símbolo garantizado: {symbol} - Precio: ${current_price:.6f}"
                        )
    ...
    def filter_symbols_by_criteria(self, metrics: dict):
        ...
        for symbol, data in metrics.items():
            try:
                ...
                if (
                    volatility < config.MIN_VOLATILITY_24H
                    or volatility > config.MAX_VOLATILITY_24H
                ):
                    continue

                if not self.capital_manager.can_trade_symbol_max_profit(
                    symbol, current_price
                ):
                    continue
        ...
        log.info(
            f"🎯 Símbolos analizados: {symbols_checked}, viables: {symbols_passed}"
        )
    ...
    def scan_universal_symbols(self):
        ...
        try:
            ...
            symbols_to_analyze = list(filtered_symbols.keys())[
                : config.MAX_SYMBOLS_TO_ANALYZE
            ]
            log.info(
                f"🎯 Analizando {len(symbols_to_analyze)} símbolos para 9.81 USDT MAXPROFIT..."
            )
            ...
            for symbol in symbols_to_analyze:
                try:
                    score, details = self.calculate_profitability_score(
            ...
            if not top_symbols:
                log.warning(
                    "⚠️ No se encontraron símbolos en el scan, usando símbolos garantizados..."
                )
            ...
            log.info(
                "🎯 SCAN UNIVERSAL COMPLETADO - Top Símbolos para 9.81 USDT MAXPROFIT:"
            )
            for i, symbol_data in enumerate(top_symbols[:12]):
                log.info(
                    f"   {i + 1}. {symbol_data['symbol']} - Score: {symbol_data['score']}/100 "
...
class MaxProfitSignalGenerator:
    ...
    def analyze_symbol_max_profit(self, symbol: str):
        ...
        try:
            ...
            if (
                current_price > config.MAX_PRICE_USDT
                or current_price < config.MIN_PRICE_USDT
            ):
                return None
            ...
            if score >= config.MIN_SIGNAL_SCORE and signal_type is not None:
                ...
                log.info(
                    f"🎯 SEÑAL MAXIMO 9.81 USDT: {symbol} {signal_type} Score: {score} "
    ...
    def _get_max_profit_technical_signal(
    ...
    def _calculate_max_profit_momentum_score(
        self, df_1m: pd.DataFrame, df_3m: pd.DataFrame
    ) -> int:
        ...
        try:
            ...
            if len(df_1m) >= 3:
                price_change_1m = (
                    (df_1m["close"].iloc[-1] - df_1m["close"].iloc[-3])
                    / df_1m["close"].iloc[-3]
                    * 100
                )
            ...
            if (
                df_1m["ema_fast"].iloc[-1] > df_1m["ema_slow"].iloc[-1]
                and df_3m["ema_fast"].iloc[-1] > df_3m["ema_slow"].iloc[-1]
            ):
                score += 2
            elif (
                df_1m["ema_fast"].iloc[-1] < df_1m["ema_slow"].iloc[-1]
    ...
    def calculate_max_profit_indicators(self, df: pd.DataFrame):
        ...
        try:
            ...
            df["atr"] = talib.ATR(
                df["high"], df["low"], df["close"], timeperiod=config.ATR_PERIOD
            )
...
class MaxProfitPositionManager:
    ...
    def close_position(self, symbol: str, quantity: float):
        ...
        try:
            if config.DRY_RUN:
                log.info(
                    f"[DRY_RUN] 🎯 Cerrando posición simulada: {symbol} {quantity}"
                )
                return {"mock": True}
...
class EnhancedProtectionSystem:
    ...
    def can_trade_max_profit(self) -> bool:
        ...
        if (
            hasattr(self, "current_balance")
            and self.current_balance < self.min_balance_protection
        ):
            log.warning(
                f"⛔ Balance por debajo del mínimo protegido: ${self.current_balance:.2f} < ${self.min_balance_protection:.2f}"
            )
            return False
        ...
        try:
            ...
            if account:
                ...
                if available_balance < config.MIN_AVAILABLE_MARGIN:
                    log.warning(
                        f"⛔ Margen disponible insuficiente: ${available_balance:.2f} < ${config.MIN_AVAILABLE_MARGIN:.2f}"
                    )
                    return False
        except Exception as e:
        ...
        if self.daily_stats["trades_count"] >= config.MAX_DAILY_TRADES:
            log.warning(
                f"⛔ Límite diario de trades: {self.daily_stats['trades_count']}/{config.MAX_DAILY_TRADES}"
            )
            return False
        ...
        if self.daily_stats["daily_pnl"] <= -config.DAILY_LOSS_LIMIT:
            ...
            log.warning(
                f"⛔ Límite pérdida diaria: ${self.daily_stats['daily_pnl']:.2f} → bloqueo {lock_minutes} min"
            )
            return False

        if self.daily_stats["daily_pnl"] >= 100.0:
            log.info(
                f"🎯 Objetivo diario MAXPROFIT alcanzado: +${self.daily_stats['daily_pnl']:.2f}"
            )
        ...
        current_time = datetime.now()
        recent_trades = [
            t for t in self.trade_times if (current_time - t).total_seconds() < 3600
        ]
    ...
    def record_trade_result(self, pnl: float, symbol: str):
        ...
        log.info(
            f"📊 ESTADO DIARIO MAXPROFIT: Trades: {self.daily_stats['trades_count']}, "
...
class UniversalScanBinanceFuturesBot:
    ...
    def start(self):
        ...
        log.info(
            f"💰 BALANCE INICIAL 9.81 USDT MAXPROFIT: {self.capital.current_balance:.2f} USDT"
        )
    ...
    def _execute_universal_scan(self):
        ...
        try:
            ...
            if top_symbols:
                ...
                log.info(
                    f"✅ SCAN completado: {len(self.current_trading_symbols)} símbolos para 9.81 USDT MAXPROFIT"
                )
            else:
            ...
            try:
                ...
                if current_time - last_balance_update > 8:
                    if self.capital.update_balance():
                        self.protection_system.set_current_balance(
                ...
                if (
                    ...
                    or len(self.current_trading_symbols) == 0
                ):
                    self._execute_universal_scan()

                has_balance = (
                    self.capital.current_balance > config.MIN_BALANCE_THRESHOLD
                )
                ...
                if self.cycle_count % 4 == 0:
                    log.info(
                        f"📈 ESTADO MAXPROFIT - Ciclo: {self.cycle_count}, "
    ...
    def _health_check(self):
        ...
        try:
            ...
            log.info(
                f"❤️ Salud del bot MAXPROFIT: Balance={'OK' if balance_ok else 'BAJO'}, "
        ...
        try:
            ...
            if account:
                ...
                if available_balance < 3.0:
                    log.warning(
                        f"⭕ Margen global insuficiente: ${available_balance:.2f}"
                    )
                    return
            ...
            for symbol in self.current_trading_symbols:
                ...
                if signal and signal["score"] >= config.MIN_SIGNAL_SCORE:
                    log.info(
                        f"🎯 Señal MAXIMO 9.81 USDT: {symbol} {signal['type']} "
    ...
    def _execute_trade(self, signal: dict):
        ...
        try:
            ...
            if not account:
                log.warning(
                    f"⭕ No se pudo obtener información de cuenta para {symbol}"
                )
                return
            ...
            if available_balance < MIN_AVAILABLE_MARGIN:
                log.warning(
                    f"⭕ Margen insuficiente para {symbol}: ${available_balance:.2f} < ${MIN_AVAILABLE_MARGIN:.2f}"
                )
                return
            ...
            if (
                ...
                or tp_price <= 0
                or entry_price <= 0
                or entry_price > config.MAX_PRICE_USDT
            ):
            ...
            if stop_distance_pct < config.MIN_STOP_DISTANCE_PCT:
                log.warning(
                    f"⭕ Stop demasiado cercano: {stop_distance_pct:.3%} < {config.MIN_STOP_DISTANCE_PCT:.3%}"
                )
                return

            log.info(
                f"🎯 EJECUTANDO TRADE MAXIMO 9.81 USDT: {symbol} {signal_type} "
            ...
            if available_balance - position_value < 1.0:
                log.warning(
                    f"⭕ Operación dejaría margen insuficiente: ${available_balance:.2f} - ${position_value:.2f} < $1.00"
                )
                return
            ...
            if not self.capital.validate_position_limits(symbol, quantity, entry_price):
                ...
                if (
                    min_position_value >= config.MIN_NOTIONAL_SAFETY
                    and min_position_value <= available_balance * 0.8
                ):
                    quantity = min_quantity_safety
                    position_value = min_position_value
                    log.info(
                        f"🔧 Usando cantidad mínima de seguridad: {quantity:.0f} = ${position_value:.2f}"
                    )
                else:
            ...
            log.info(
                f"✅ Orden EJECUTADA MAXIMO 9.81 USDT: {symbol} {side} {quantity:.0f} = ${position_value:.2f}"
            )
    ...
    def _manage_position(self, symbol: str):
        ...
        try:
            ...
            if (side == "LONG" and current_price >= tp_price) or (
                ...
                log.info(
                    f"🎯 Take Profit MAXIMO 9.81 USDT: {symbol} PnL: {pnl_pct:.2f}%"
                )
    ...
    def _close_position_with_profit(self, symbol: str, pnl_pct: float):
        ...
        try:
            ...
            if result:
                ...
                log.info(
                    f"💰 Posición cerrada con GANANCIA MAXIMO 9.81 USDT: {symbol} +{pnl_pct:.2f}% (+${pnl_usdt:.2f})"
                )
    ...
    def _close_position_with_loss(self, symbol: str, pnl_pct: float):
        ...
        try:
            ...
            if result:
                pnl_usdt = (pnl_pct / 100) * self.capital.current_balance
                log.info(
                    f"📉 Posición cerrada con PÉRDIDA MAXIMO 9.81 USDT: {symbol} {pnl_pct:.2f}% (${pnl_usdt:.2f})"
                )
                self.protection_system.record_trade_result(pnl_usdt, symbol)
                self.position_manager.remove_bot_managed_position(symbol)
...
def api_status():
    ...
    try:
        bot_status = {
            ...
            "active_positions": (
                len(bot.position_manager.active_positions) if "bot" in globals() else 0
            ),
            "trading_symbols_count": (
                len(bot.current_trading_symbols) if "bot" in globals() else 0
            ),
...
def handle_connect():
    ...
    socketio.emit(
        "log_update",
        {
            "message": "✅ Cliente conectado - Modo tiempo real activado",
            "level": "info",
        },
    )
        ...
        while True:
            try:
                if "bot" in globals() and bot.running:
                    ...
                    for symbol, position in active_positions.items():
                        try:
                            current_price = bot.position_manager.get_current_price(
                                symbol
                            )
                            entry_price = position.get("entry_price", 0)
                            amount = position.get("amount", 0)
                            ...
                            positions_data.append(
                                {
                                    ...
                                    "pnl_percent": pnl_percent,
                                    "leverage": position.get("leverage", 1),
                                    "liquidation_price": position.get(
                                        "liquidation_price", 0
                                    ),
                                }
                            )
                    ...
                    performance_data = {
                        "daily_trades": protection.daily_stats["trades_count"],
                        "daily_pnl": protection.daily_stats["daily_pnl"],
                        "consecutive_losses": protection.daily_stats[
                            "consecutive_losses"
                        ],
                        "consecutive_wins": protection.daily_stats["consecutive_wins"],
...
if __name__ == "__main__":
    try:
        log.info(
            "🚀 INICIANDO BOT MAXIMO RENDIMIENTO PARA 9.81 USDT - SISTEMA TIEMPO REAL"
        )
