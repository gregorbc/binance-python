...
with open(ta_func_header) as f:
    ...
    for line in f:
        if line.startswith("TA_LIB_API"):
            line = line[10:]
        line = line.strip()
        if tmp or line.startswith("TA_RetCode TA_") or line.startswith("int TA_"):
...
for f in functions:
    ...
    for arg in args:
        ...
        if var.endswith("[]"):
            ...
            var = cleanup(var)
            default_arg = arg.split()[-1][
                len("optIn") :
            ]  # chop off typedef and 'optIn'
            default_arg = (
                default_arg[0].lower() + default_arg[1:]
            ...
            if arg.startswith("double"):
                ...
                print(
                    "int %s=%s" % (var, defaults.get("matype", 0)), end=" "
                )  # TA_MAType_SMA
            else:
                assert False, arg
            if "[, " not in docs:
    ...
    if documentation:
        ...
        documentation = documentation.split("\n")[
            2:
        ]  # discard abstract calling definition
    ...
    print(
        "    begidx = check_begidx%s(length, %s)"
        % (len(inputs), ", ".join("<double*>(%s.data)" % s for s in inputs))
    )
