...
def test_pandas(ford_2012):
    ...
    expected_k, expected_d = func.STOCH(
        ford_2012["high"], ford_2012["low"], ford_2012["close"]
    )  # 5, 3, 0, 3, 0
...
def test_pandas_series(ford_2012):
    ...
    expected = pandas.Series(func.SMA(ford_2012["close"], 10), index=input_df.index)
    ...
    expected = pandas.Series(
        func.BBANDS(ford_2012["close"], 10)[1], index=input_df.index
    )
    pandas.testing.assert_series_equal(output, expected)
...
def test_SMA(ford_2012):
    ...
    assert_array_equal(
        expected, abstract.Function("sma", ford_2012, 10, price="open").outputs
    )
    assert_array_equal(
        expected, abstract.Function("sma", price="low")(ford_2012, 10, price="open")
    )
    assert_array_not_equal(
        expected, abstract.Function("sma", ford_2012, 10, price="open")(timeperiod=20)
    )
    assert_array_not_equal(
        expected, abstract.Function("sma", ford_2012)(10, price="close")
    )
    assert_array_not_equal(
        expected, abstract.Function("sma", 10)(ford_2012, price="high")
    )
    assert_array_not_equal(
        expected, abstract.Function("sma", price="low")(ford_2012, 10)
    )
...
def test_STOCH(ford_2012):
    ...
    expected_k, expected_d = func.STOCH(
        ford_2012["high"], ford_2012["low"], ford_2012["close"], 15
    )
    ...
    expected_k, expected_d = func.STOCH(
        ford_2012["high"], ford_2012["low"], ford_2012["close"], 15, 5, 1, 5, 1
    )
...
def test_doji_candle(ford_2012):
    expected = func.CDLDOJI(
        ford_2012["open"], ford_2012["high"], ford_2012["low"], ford_2012["close"]
    )
...
def test_parameter_type_checking(ford_2012):
    ...
    expected_error = re.escape(
        "Invalid parameter value for timeperiod (expected int, got float)"
    )
...
def test_threading():
    ...
    df_short = pd.DataFrame(
        data_short, columns=["open", "high", "low", "close", "volume"]
    )
    df_long = pd.DataFrame(
        data_long, columns=["open", "high", "low", "close", "volume"]
    )
